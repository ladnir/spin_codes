#!/usr/bin/env python3
"""Decide whether the 12-node autonomous observation code has weight at most 138."""
...
GOAL02_PART1 = CANDIDATE / "receipts" / "goal02_tail_list_primary.json"
OUTPUT = CANDIDATE / "receipts" / "goal02_c12_native_xor.json"
...
def systematic_right_columns() -> tuple[int, ...]:
    ...
    if any(apply_columns(left, inverse[bit]) != 1 << bit for bit in range(64)):
        raise RuntimeError("Goal 02 C12: systematic reconstruction failed")
...
def main() -> None:
    ...
    if args.timeout_seconds <= 0:
        raise SystemExit("Goal 02 C12: timeout must be positive")
    ...
    payload = {
        "schema": "riffle-packetmul-wrapmul-2lap-g4-goal02-c12-native-xor-v1",
        ...
        "goal02_part1_sha256": digest(GOAL02_PART1),
        "code": {
        ...
        "decision_problem": {
            ...
            "four_node_block_weight_range": [36, 66],
        },
        "encoding": {
        ...
        "result": result,
        "scope_limitation": (
            "UNSAT proves the exact decision in this solver but has no independently "
            "checkable proof trace. UNKNOWN records only a timeout."
        ),
    }
    if model is not None:
        ...
        if information == 0 or weight > REJECTED_WEIGHT:
            raise RuntimeError("Goal 02 C12: SAT model replay failed")
