\section{Structured local-code outer interface}
\label{sec:structured-local-code}
\label{sec:local-code-outer}

The second main construction line replaces sliding random bands by short local
codes supplied through explicit spectra or certified spectrum envelopes.  This
section defines the local-code-independent outer interface and the finite
certificate template used by the later RM/EBCH instantiation.  The local code is
an input to the theorem; RM, BCH, or another code with a certified spectrum are
possible choices.

\subsection{Direct-sum outer spectrum}
...
words gives \(\mu_{\mathrm{block}}(\delta)\), and Markov's inequality gives the
minimum-distance bound.
\end{proof}

\subsection{Finite certificate template}
