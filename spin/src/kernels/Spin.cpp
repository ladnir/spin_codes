...
template<class T> static void shuffle(T* p,std::size_t n,setup::Words& words,const setup::Divisor* divisors) {
    std::iota(p,p+n,T{0});
    for(std::size_t i=n;i>1;--i) std::swap(p[i-1],p[divisors[i].sample(words,i)]);
}
...
Spin::Spin(Configuration c,MessageLength length,u64 routeSeed,u64 coefficientSeed,unsigned tileRows,BchBackend backend,bool compactSetup,std::vector<u32> preparedRoute)
    :mConfig(c),mK(0),mTileRows(tileRows),mRoute(std::move(preparedRoute)) {
    ...
    const bool keep32=!compactSetup || !packed24Available();
    if(mBchBackend==BchBackend::Avx512) {if(packed24Available()) mBchOffsets24.resize(3*n+4);if(keep32)mBchOffsets32.resize(n);}
    mPartialTile=(rows%mTileRows)!=0;
    if(rows%step()) throw std::invalid_argument("incomplete region epochs");
    // Private experiment hook. The public Code API never supplies a route.
    // Experimental callers must check bijectivity/row-region uniqueness first.
    if(!mRoute.empty() && mRoute.size()!=n)throw std::invalid_argument("prepared route size mismatch");
    if(mRoute.empty()) {
    std::vector<u8> regionCoordinate(n);
    setup::Words routeWords(routeSeed);
    std::vector<setup::Divisor> divisors(std::max<std::size_t>(256,rows)+1);
    for(std::size_t d=2;d<divisors.size();++d)divisors[d]=setup::Divisor(d);
    std::array<u8,256> coordinateRegion;
    for(std::size_t row=0;row<rows;++row) {
        shuffle(coordinateRegion.data(),256,routeWords,divisors.data());
    ...
    for(unsigned region=0;region<256;++region) {
        shuffle(positions.data(),rows,routeWords,divisors.data());
        for(std::size_t row=0;row<rows;++row)
    ...
    if(keep32){mSlots32.resize(n);mOffsets32.resize(n);}
    ...
    for(std::size_t inner=n;inner-->0;) {
        ...
        if(keep32){mSlots32[inner]=slot;mOffsets32[slot]=local;}
        if(packed24Available()) {pack(mSlots24.data()+3*inner,slot); pack(mOffsets24.data()+3*slot,local);}
        if(mBchBackend==BchBackend::Avx512) {
            if(keep32)mBchOffsets32[slot]=packFour(local);if(packed24Available()) pack(mBchOffsets24.data()+3*slot,packFour(local));
        }
    }
    switch(c) {
...
template<class Map> void Spin::setupInner(u64 seed) {
    ...
    if constexpr(isImtMap<Map>) {
        ...
        for(std::size_t e=0;e<rounds*epochs;++e) {
            u32 u;do u=u32(words())&((1U<<Map::S)-1);while(!u);
            u32 v=u32(words())&((1U<<Map::S)-1);
