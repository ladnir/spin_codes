...
\subsection{Envelope interpretation note}

The compiled dense-inner section keeps the packaged global, run-aware, and
small-weight envelopes.  Their roles are:
\begin{itemize}
\item the global envelope is designed for the linear-weight regime; its first
term decays exponentially in the input weight, its second term exposes the
ON\(\to\)OFF memory penalty \(N2^{-m}\), and its third term is a pure
large-deviation term in \(N\);
\item the run-aware envelope preserves the exact interleaved run-count
combinatorics and turns the OFF mechanism into the multiplicative term
\((L2^{-m})^r\);
\item the fixed-tap small-weight envelope specializes the isolated-start regime,
where genuine termination candidates must be aligned with later input-one
positions.
\end{itemize}

\subsection{Demoted geometric-envelope bookkeeping}

The compiled scalar dense-inner section keeps the constant-factor geometric envelope because it is the form consumed by
the dense+dense integration theorem.  The more general polynomial-prefactor variant and the explanatory OFF-term remark
are preserved here.
...
Let $p_w(\delta)$ denote the slice-to-tail probability from the dense-inner global envelope.
Assume the dense-inner tail bound gives
\begin{equation}
...
\subsection{Demoted fixed-tap candidate bookkeeping}

The following fixed-tap lemmas are useful for finite scalar diagnostics and for the older isolated-slice correction
program, but they are not consumed by the current random sliding dense asymptotic theorem. They were therefore moved out
of the compiled scalar dense spine during the cleanup pass.
...
\begin{proof}
Condition on the first input-one position \(\tau=t\), and put \(S=N-t+1\). The first output is deterministically one,
and on \(\mathcal{S}_{\mathrm{fin}}\) the remaining suffix stays on a surviving ON episode, so the suffix lower tail is
bounded by the corresponding fair-coin tail. Under the uniform weight-\(h\) law,
\[
\PP[S(U)=S\mid \wt(U)=h]=\frac{\binom{S-1}{h-1}}{\binom{N}{h}},
\]
and averaging over \(S\) gives the displayed bound.
\end{proof}
...
\begin{proof}
Both a genuine start and a genuine termination require \(u_t=1\); the termination statement follows from
Lemma~\ref{lem:fixedtap-termination-needs-input-one}. The two cases are disjoint at a fixed time because \(s_t\)
cannot be both zero and nonzero.
\end{proof}
...
\begin{proof}
Expose the trajectory from left to right. Each requested termination imposes \(m-1\) predictable free-time zero
constraints after the leading one in the critical state, unless the requested pattern is already impossible. By
Lemma~\ref{lem:dense-on-time-domination}, each requested termination costs at most \(2^{-(m-1)}\) conditional on the
compatible past. Multiplying over the \(r\) requested positions gives the claim.
\end{proof}
...
\[
1\le x_1<\cdots<x_h\le N,\qquad x_{a+1}-x_a\ge 2.
\]
...
\]
Since \(x_b-x_a\le L\) implies \(z_b-z_a\le L\), linearity of expectation proves the displayed bound.
\end{proof}

\subsection{Demoted first-start and single-run diagnostics}

These position-sensitive scalar dense diagnostics are useful for finite calibration and for understanding why the
\(h=1\) inner slice has a \(2\delta\)-type late-start base. They are not used by the current analytic dense+dense
theorem, so the compiled main spine keeps only the coarser three-term tail and the run/forced-termination envelopes.

\begin{corollary}[Zero-input gaps survive in the fixed-tap inner]
\label{cor:dense-gap-survival}
Fix an interval of \(g\ge0\) consecutive zero inputs, and condition on the event that the encoder is ON at the start of
...
\(\PP[\Binomial(N-t,\tfrac12)\le d]\). Average over \(T\), then add the OFF event for the final display.
\end{proof}
...
If there is no later ON\(\to\)OFF transition and \(N-T\ge \lceil2\delta N\rceil\), the low-weight suffix event is bounded
by the displayed binomial tail. The complementary late-start event has probability at most \(\lceil2\delta N\rceil/N\).
Finally, Lemma~\ref{lem:dense-off-requires-mzeros} and the same union bound as in Lemma~\ref{lem:dense-three-term-tail}
give the \(N2^{-m}\) OFF term.
\end{proof}
