...
int main(int argc,char** argv) {
    try {
        ...
        if(trials<3 || !(trials&1) || layout>1 || (onlyM && onlyM!=16 && onlyM!=18 && onlyM!=20))
            throw std::invalid_argument("usage: benchmark odd_trials>=3 tile_rows layout[0,1] [m=16,18,20]");
#ifdef __linux__
        ...
        for(unsigned m:{16U,18U,20U}) {
            ...
            for(unsigned c=0;c<4;++c) {
                ...
                Spin code(static_cast<Configuration>(c),m,1,2,tile);
                ...
                std::vector<block> in(code.codeBlocks()),out(code.messageBlocks());
                u64 seed=123;
                for(auto& v:in) {auto lo=splitmix(seed);auto hi=splitmix(seed);v=block(hi,lo);}
                for(unsigned i=0;i<3;++i) code.encodeUnchecked(in.data(),out.data(),w,route);
                std::vector<double> samples; samples.reserve(trials);
                for(unsigned i=0;i<trials;++i) {
                    begin=std::chrono::steady_clock::now();
                    code.encodeUnchecked(in.data(),out.data(),w,route);
                    samples.push_back(elapsed(begin));
                    sink=out[(i*997)%out.size()].get<u64>()[0];
                }
                std::sort(samples.begin(),samples.end());
                u64 hash=0xcbf29ce484222325ULL;
                for(const auto& v:out) for(auto x:v.get<u64>()) {hash^=x;hash*=0x100000001b3ULL;}
                std::cout<<std::setprecision(10)<<"{\"configuration\":\""<<code.name()<<"\",\"m\":"<<m
                    <<",\"tile_rows\":"<<code.tileBlocks()/256<<",\"layout\":\""<<(layout?"indices32":"packed24")
