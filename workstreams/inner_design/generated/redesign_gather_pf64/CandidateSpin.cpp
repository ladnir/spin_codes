...
const char* Spin::name() const noexcept {
    if(mOuter==Outer::Bch128x32) return "asymmetric_greedy3_2_exact_gather_pf64";
        ...
        for(std::size_t j=0;j<n;j+=256) {
            for(unsigned p=0;p<256;p+=8) {
                auto gather=[&]<unsigned K>() {
                    ...
                    if(position+64<n) {
                        const auto future=Packed?unpack(mSlots24.data()+3*(position+64)):mSlots32[position+64];
