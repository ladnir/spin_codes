...
const char* Spin::name() const noexcept {
    if(mOuter==Outer::Bch128x32) return "routeopt_scatter128";
...
template<class Map,bool Packed,bool Quarter> void Spin::run(const block* in,block* out,Workspace& w) const {
    ...
    for(std::size_t base=0;base<n;base+=tileSize) {
        for(std::size_t j=0;j<tileSize;j+=8) {
            auto point=[&]<unsigned K>() {
                const auto pos=base+j+K;
                if(j+K+128<tileSize) {
                    const auto future=Packed?unpack(mOffsets24.data()+3*(pos+128)):mOffsets32[pos+128];
                    __builtin_prefetch(tile+future,0,3);
                }
