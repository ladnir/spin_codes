...
Spin::Spin(Configuration c,unsigned exponent,u64 routeSeed,u64 coefficientSeed,unsigned tileRows,Outer outer)
    ...
    if(c!=Configuration::T128S19 || outer!=Outer::Bch256x128)
...
const char* Spin::name() const noexcept {
    if(mOuter==Outer::Bch256x128) return "t128_s19_weight5_seed0_r1";
...
template<class Map,bool Packed,bool Quarter> void Spin::run(const block* in,block* out,Workspace& w) const {
    ...
    asymmetricReverse<Map,true,W5_MASKED>(in,codeBlocks(),mFieldRows.data(),[&](std::size_t i,block v) {
    ...
    for(std::size_t base=0;base<n;base+=tileSize) {
        for(std::size_t j=0;j<tileSize;++j) {
            if constexpr(W5_PREFETCH>0) if(j+W5_PREFETCH<tileSize) {
                const auto future=Packed?unpack(mOffsets24.data()+3*(base+j+W5_PREFETCH)):mOffsets32[base+j+W5_PREFETCH];
...
void Spin::validateSetup() const {
    if(!mCompacted && mOuter==Outer::Bch256x128) {
