"""Replay causal and coarsened IMT covers with exact integer coverage."""
import argparse
import json
import math
...
import numpy as np
import parameter_three_type_dense as three
import type_box_coverage


def verify(path,output):
    base = three.prior.prior.prior.base
    model = base.grid.ladder.model
    saved = model.base.read(path)
    model.authenticate(saved)
    classes = {'BINARY64_IMT_CAUSAL_DENSE_COVER':three.prior.Dense,
               'BINARY64_IMT_THREE_TYPE_DENSE_COVER':three.Dense}
    b,t,s,exponent = saved['geometry']
    checker = classes[saved['status']](t,s,b,exponent,[])
