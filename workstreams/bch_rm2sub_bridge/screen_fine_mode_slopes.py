...
def run(tilt,grid,tag):
    ...
    breaks=sorted(set([0,1,2,3,4,6,8,12,16,24]+list(range(32,8193,32))))
    ...
    print('Fine-slope modes',[(i,round(float(max(row).log()),2)) for i,row in enumerate(modes) if max(row)>0],flush=True)
    ...
    base.write_new(base.HERE/'generated'/f'fine_mode_slopes_{tag}_screen.json',dict(status='SHARED_SLOPE_MODE_SCREEN_ONLY',
        tilt_tenth=tilt,grid=grid,segments=segments,rows=rows,
        coefficient_logs=[float(c.log()) if c>0 else None for c in coefficients],
