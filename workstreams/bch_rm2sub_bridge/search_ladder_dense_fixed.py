"""Complete covers using a fixed iid reference and an explicit type-count cost."""
import argparse
...
import ladder_dense_fixed_reference as dense

base,core = old.base,old.core
POLICY = 'two-tilt-fixed-reference-type-count-balanced-v1'
...
def run(directory,m,minimum,seconds,nodes,verify=False):
    ...
    while heap and used < nodes and time.monotonic()-started < seconds:
        ...
        if node['power'] > -80:
            lo,hi,a,b = old.geometry(node)
            if lo == hi and b-a <= F(1,1 << 20):
                # Preserve a valid unresolved leaf instead of repeatedly
                # subdividing a point that needs a different witness family.
                parked.append(key)
                continue
