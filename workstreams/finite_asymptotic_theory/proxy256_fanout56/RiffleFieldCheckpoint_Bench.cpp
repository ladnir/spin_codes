...
int main(int argc, char** argv)
{
	try
	{
		...
		const bool fanout56ProxyOnly =
			argc > 2 && std::string(argv[2]) == "fanout56-proxy";
#if defined(LIBOTE_RIFFLE_BCH_CIRCUIT_PAIR_BENCH)
		...
		if (fanout56ProxyOnly)
		{
			constexpr u64 rm2Epochs = N / RiffleRm2SubS19Transpose::stepBlocks;
			constexpr u64 proxyLayers = RiffleExactPermFieldCheckpoint::parityFanoutLayers;
			constexpr u64 fanoutXorsPerLayer = 31 + 33 - 1;
			verifyRm2SubS19Transpose();

			RiffleRm2SubS19Transpose inner;
			RiffleExactPermFieldCheckpoint promoted;
			const double proxySetupMs = milliseconds([&] {
			...
			std::vector<block> stagedZero(K);
			std::vector<block> staged56(K);
			std::vector<block> candidateZero(K);
			std::vector<block> candidate56(K);
			...
			{
				block* word0 = outerWord.data() + outer * OuterLength;
				block* word1 = word0 + OuterLength;
				ExtendedBch256x128Eq3::transposeBlock2(
					word0,
					word1,
					stagedZero.data() + outer * OuterDimension,
					stagedZero.data() + (outer + 1) * OuterDimension);

				const u64 base0 = outer * proxyLayers;
				const u64 base1 = (outer + 1) * proxyLayers;
				for (u64 layer = proxyLayers; layer-- > 0;)
				{
					const auto& fanout0 = promoted.parityFanouts()[base0 + layer];
					const auto& fanout1 = promoted.parityFanouts()[base1 + layer];
				...
				ExtendedBch256x128Eq3::transposeBlock2(
					word0,
					word1,
					staged56.data() + outer * OuterDimension,
					staged56.data() + (outer + 1) * OuterDimension);
			}

			promoted.dualEncodePackedInnerFanoutUnchecked<32, 0>(
				source.data(), candidateZero.data(), workspace, inner);
			promoted.dualEncodePackedInnerFanoutUnchecked<32, proxyLayers>(
				source.data(), candidate56.data(), workspace, inner);
			if (!equalBlocks(stagedZero.data(), candidateZero.data(), K))
				throw std::runtime_error("zero-layer proxy disagrees with staged oracle");
			if (!equalBlocks(staged56.data(), candidate56.data(), K))
				throw std::runtime_error("56-layer proxy disagrees with staged oracle");
			if (equalBlocks(candidateZero.data(), candidate56.data(), K))
				throw std::runtime_error("zero-layer and 56-layer proxy outputs agree");

			for (u64 warmup = 0; warmup < 3; ++warmup)
			{
				promoted.dualEncodePackedInnerFanoutUnchecked<32, 0>(
					source.data(), candidateZero.data(), workspace, inner);
				promoted.dualEncodePackedInnerFanoutUnchecked<32, proxyLayers>(
			...
			std::vector<double> fanout56Samples;
			zeroSamples.reserve(trials);
			fanout56Samples.reserve(trials);
			auto runZero = [&] {
				return milliseconds([&] {
					promoted.dualEncodePackedInnerFanoutUnchecked<32, 0>(
						source.data(), candidateZero.data(), workspace, inner);
			...
			for (u64 trial = 0; trial < trials; ++trial)
			{
				if (trial & 1)
				{
					fanout56Samples.push_back(runFanout56());
					zeroSamples.push_back(runZero());
				}
				else
				{
					zeroSamples.push_back(runZero());
			...
			benchmarkSink ^= checksum(candidateZero.data(), candidateZero.size());
			benchmarkSink ^= checksum(candidate56.data(), candidate56.size());
			std::cout << std::fixed << std::setprecision(6)
				<< "construction=Riffle-BCH256x128-RM2SubS19-Fanout56Proxy"
				<< " mode=fanout56-proxy"
				<< " benchmark_cpu=" << benchmarkCpu
				<< " K=" << K << " N=" << N
				<< " fanout_layers=" << proxyLayers
				<< " fanout_xors_per_call="
				<< OuterBlocks * proxyLayers * fanoutXorsPerLayer
				<< " fanout_schedule_mib="
				<< double(OuterBlocks * proxyLayers * 64) /
					double(u64{1} << 20)
				<< " setup_ms=" << proxySetupMs
				<< " trials=" << trials
				<< " correctness=staged-zero+staged-56+rm2sub-PASS\n";
			printSamples("zero_layers", zeroSamples);
			printSamples("fanout56_layers", fanout56Samples);
			std::cout << "fanout56_over_zero="
				<< median(fanout56Samples) / median(zeroSamples)
				<< " incremental_ms="
				<< median(fanout56Samples) - median(zeroSamples)
