"""Independent 90-digit replay of every positive zero-state grid witness."""
import json
import math
...
def main():
    ...
    path = HERE/'bch_zero_state_lower_grid_v2.json'
    ...
    selected = [r for r in payload['rows'] if r.get('first_moment_lower_bits', -math.inf) > 0]
    checks = []; cache = {}
    with mp.workdps(90):
        def log_choose(n, k):
            key = (n, k)
        ...
        for row in selected:
            ...
            logs = [mp.log(n) if n else mp.ninf for n in kernel]
            def regional(weight):
                a = 2*(weight//(2*epochs)); h = (weight-a*epochs)//2
                value = log_choose(epochs, h)-log_choose(length, weight)
                if epochs-h:
                    value += (epochs-h)*logs[a]
                if h:
                    value += h*logs[a+2]
                return value
    ...
    result = dict(status='ALL_POSITIVE_GRID_WITNESSES_REPLAYED', decimal_digits=90, checks=checks,
                  ...
                  source_sha256={str(Path(__file__)): study.sha(Path(__file__)), str(path): study.sha(path)},
    ...
    (HERE/'bch_zero_state_verification_v2.json').write_text(json.dumps(result, indent=2)+'\n')
