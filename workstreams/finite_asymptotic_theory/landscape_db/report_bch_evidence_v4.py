...
def main():
    ...
    for row in sparse:
        ...
        rows.append(result)
    path = HERE/'bch_engineering_evidence_v4.csv'
    ...
    fig.savefig(HERE/'bch_q1_vs_dense_tradeoff_v4.png', dpi=180)
    plt.close(fig)
    ...
    fig.savefig(HERE/'bch_full_bound_coverage_v4.png', dpi=180)
    plt.close(fig)
    ...
    fig.suptitle('Full BCH bounds at message-size anchors: T=64, S=20, relative distance 10%\nFull bounds are shown only at replayed geometries; losses compare optimized bounds, not true probabilities.', fontsize=13)
    fig.savefig(HERE/'bch_full_bound_k_scaling_v4.png', dpi=180)
    plt.close(fig)
    ...
    (HERE/'bch_engineering_evidence_v4.json').write_text(json.dumps(summary, indent=2)+'\n')
