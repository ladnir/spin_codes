...
import joint_dense_witness_v1 as fast
...
def main():
    ...
    seed_path = HERE/f'bch_dense_seed_b{block}_t{t}_s{s}_e{e}_q{args.minimum}.json'
    ...
    model = fast.JointCoverRefiner(counts, block, t, s, ac, config['kernel_counts'], length, raw['bands'])
    directory = HERE/f'bch_dense_v6_b{block}_t{t}_s{s}_e{e}_q{args.minimum}'
    directory.mkdir(exist_ok=True)
    old_directory = HERE/f'bch_dense_v3_b{block}_t{t}_s{s}_e{e}_q{args.minimum}'
    resume = directory/'checkpoint.json'
    if not resume.exists():
        resume = HERE/f'bch_dense_v5_b{block}_t{t}_s{s}_e{e}_q{args.minimum}/checkpoint.json'
    if resume.exists():
    ...
    for index, root in enumerate(raw['selected_boxes']):
        ...
        for leaf in selected:
            current = checks.canonical_witness(model, leaf)
    ...
    for path in (Path(__file__), Path(fast.__file__), Path(fast.fast.__file__), Path(previous.__file__), Path(checks.__file__), Path(previous.base.__file__),
    ...
    while heap and used < args.maximum_refinements and total() > target:
        ...
        children = previous.base.transfer.typed.split_box(improved['lower'], improved['upper'], corners)
        if children:
            prepared = []
        ...
        if used % 50 == 0:
            ...
            snapshot = dict(status='PARTIAL_CONTINUOUS_DENSE_REFINEMENT', source_sha256=dependencies,
    ...
    payload = dict(status='BINARY64_COMPLETE_DENSE_INTERVAL', source_sha256=dependencies, arguments=vars(args),
