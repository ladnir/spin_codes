...
def main():
    ...
    directory = HERE/f'bch_dense_v9_b{block}_t{t}_s{s}_e{e}_q{args.minimum}'
    directory.mkdir(exist_ok=True)
    old_directory = None
    resume = directory/'checkpoint.json'
    if not resume.exists():
        resume = HERE/f'bch_dense_v8_b{block}_t{t}_s{s}_e{e}_q{args.minimum}/checkpoint.json'
    if resume.exists():
    ...
    for path in (Path(__file__), Path(fast.__file__), Path(fast.density.__file__), Path(fast.joint.__file__), Path(fast.joint.fast.__file__), Path(previous.__file__), Path(checks.__file__), Path(previous.base.__file__),
