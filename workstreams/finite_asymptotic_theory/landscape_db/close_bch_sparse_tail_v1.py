"""Fine tilt search and fixed-composition covers for BCH Q5..64."""
import argparse
import hashlib
import json
import math
...
def main():
    ...
    models = [transfer.CompositionBoxes(counts, block, bands,
    ...
    directory = HERE/f'bch_sparse_tail_v1_b{block}_t{t}_s{s}_e{exponent}'
    ...
    for ordinal, tilt in enumerate(tilts):
        ...
        if path.exists():
            ...
            lam = math.exp(tilt)
            region = transfer.region_logs(epoch.at(lam, maximum), t, length, maximum)
            values = []
            for model in models:
    ...
    payload = dict(status='BINARY64_COMPLETE_SPARSE_INTERVAL', source_sha256=dependencies,
                   ...
                   occupation_min=5, occupation_max=maximum, log_upper=total, margin_bits=-total/LN2,
                   occupations=results, limitations=['Only Q5 through the displayed maximum are covered.',
