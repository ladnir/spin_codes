"""Sequential resumable occupation grid using deterministic spectrum bands.
...
def run_batch(directory,rows,arguments,sources):
    ...
    fields=list(rows[0])+['occupation','setup_event_id','setup_failure_bits','witness_shift']
    ...
    for (e,b,d),models in sorted(geometries.items()):
        ...
        for row in models:
            if row['outer_model']=='random-ensemble-average':
                caps=transfer.random_spectrum_caps(b,d,arguments['setup_failure_bits'])
                payload=dict(block_bits=b,dimension=d,setup_failure_bits=arguments['setup_failure_bits'],
                             counts=caps,method='simultaneous Markov shell caps; one reused full-rank subspace')
        ...
        for tilt in arguments['log_surprisals']:
            ...
            for index,multiplicities in enumerate(counts):
                for shift in arguments['shifts']:
                    values=transfer.occupation_bounds(regions,multiplicities,b,L,cutoff,lam,shift,arguments['bands'])
                    improve=values<best[index]
                    best[index,improve]=values[improve]
                    witnesses[index,improve,0]=tilt
                    witnesses[index,improve,1]=shift
        for index,row in enumerate(models):
            for q in range(2,maximum+1):
                ...
                tilt,shift=map(float,witnesses[index,q-1])
                result=dict(row,occupation=q,margin_bits=margin,dominant_weight='',
                            ...
                            witness_shift=shift,setup_event_id=events[index] or '',
                            setup_failure_bits=arguments['setup_failure_bits'] if events[index] else '',
                            notes='Activation-aware adaptive band bound; each integer occupation evaluated; nearest binary64; no outward certificate.')
                if events[index]:
    ...
    receipt=dict(schema='activation-aware-occupation-grid-v1',status='BINARY64_DIAGNOSTIC',
...
def main():
    ...
    parser.add_argument('--log-surprisals',nargs='+',type=float,default=[-8.,-7.,-6.,-5.,-4.,-2.,0.])
    parser.add_argument('--shifts',nargs='+',type=float,default=[-2.,0.,2.,4.])
    parser.add_argument('--bands',type=int,default=8)
    parser.add_argument('--setup-failure-bits',type=int,default=60)
    args=parser.parse_args()
    if (args.maximum_occupation<2 or args.bands<1 or args.setup_failure_bits<0
    ...
    for path in (Path(__file__),Path(transfer.__file__),Path(coverage.__file__),Path(grid.__file__)):
