...
def main():
    ...
    for b, t, s, e in dict.fromkeys(args.geometry):
        ...
        def run(script, extra, stage):
            log = HERE/f'bch_batch_v2_{tag}_{stage}.log'
        ...
        maximum = min(256, length)
        sparse = HERE/f'bch_sparse_tail_v2_{tag}_q5_{maximum}/cover.json'
        if not sparse.exists():
            run('close_bch_sparse_tail_v2.py', ['--minimum', '5', '--maximum', str(maximum)], 'sparse')
        if minimum > 257 and length >= 257:
            upper = min(length, minimum-1)
            bridge = HERE/f'bch_sparse_tail_v3_{tag}_q257_{upper}/cover.json'
            if not bridge.exists():
                run('close_bch_sparse_tail_v3.py', ['--minimum', '257', '--maximum', str(upper)], 'bridge')
        refined = HERE/f'bch_dominance_v2/{tag}.json'
        extra = ['--dense-minimum', str(minimum)]
        if refined.exists():
            extra += ['--sparse-checkpoint', str(refined)]
