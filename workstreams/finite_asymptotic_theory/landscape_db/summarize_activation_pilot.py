...
def main():
    ...
    rows = [dict(r) for r in db.execute(
        "SELECT * FROM landscape WHERE transfer_review_status='activation_aware' AND comparison_eligible=1 ORDER BY outer_id,message_exponent,step_bits,state_bits")]
    if not rows:
        raise ValueError('no activation-aware observations')
    if any(r['dominant_witness_at_grid_edge'] for r in rows):
        raise ValueError('preferred screen still reaches a tilt-grid boundary')
    for row in rows:
