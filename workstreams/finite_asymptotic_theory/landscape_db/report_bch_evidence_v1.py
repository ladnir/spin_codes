...
def main():
    ...
    for row in sparse:
        ...
        rows.append(result)
    path = HERE/'bch_engineering_evidence_v1.csv'
    ...
    for column, block in enumerate((64, 128)):
        ...
        bottom.set(title='Zero-state trajectories expose a different tradeoff', xlabel='State bits s',
                   ylabel='log₂(bad-word first-moment lower bound) / output bits')
    ...
    fig.savefig(HERE/'bch_q1_vs_dense_tradeoff.png', dpi=180)
    ...
    (HERE/'bch_engineering_evidence_v1.json').write_text(json.dumps(summary, indent=2)+'\n')
