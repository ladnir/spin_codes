...
def main():
    # The report runs after producers finish; authenticate each shared file once.
    digests = {}

    def sha(path):
        path = Path(path).resolve()
        if path not in digests:
            digests[path] = study.sha(path)
        return digests[path]
    ...
    if verification['source_sha256'][str(lower_path)] != sha(lower_path):
        raise ValueError('zero-state verification is stale')
    receipt = json.loads((HERE/'bch_dominance_sparse.json').read_text())
    if sha(HERE/'bch_dominance_sparse.csv') != receipt['csv_sha256']:
    ...
    if exact_path.exists() and exact_verification.exists():
        ...
        if replay['source_sha256'][str(exact_path)] != sha(exact_path):
    ...
    for row in sparse:
        ...
        lower_gap = lower_bits+q1 if obstructed else None
        lower_ratio = (lower_gap+math.log(-math.expm1(-lower_gap*math.log(2)))/math.log(2)
                       if lower_gap is not None and lower_gap > 0 else None)
        ...
        result.update(q1_margin_bits=q1, q2_q4_margin_bits=sparse_margin,
                      ...
                      higher_to_q1_log2_lower_bound=lower_ratio,
                      full_margin_upper_from_first_moment_lower_bits=-lower_bits if obstructed else None,
        ...
        rows.append(result)
    path = HERE/'bch_engineering_evidence_v8.csv'
    ...
    fig.savefig(HERE/'bch_q1_vs_dense_tradeoff_v8.png', dpi=180)
    plt.close(fig)
    ...
    ax.legend(handles=[Patch(facecolor=c, label=l) for code, (c, l) in enumerate(zip(palette, labels))
                       if np.any(matrix == code)],
              loc='upper center', bbox_to_anchor=(.5, -.18), ncol=2, frameon=False)
    fig.savefig(HERE/'bch_full_bound_coverage_v8.png', dpi=180)
    plt.close(fig)
    model_checks = []
    ...
    for column, block in enumerate((64, 128)):
        ...
        if verified:
            bottom.set_title(f"Largest displayed correction: {max(r['full_aggregation_penalty_bits'] for r in verified):.2g} bits", fontsize=10)
        for r in verified:
            if r['message_exponent'] not in (12, 16, 20, 24, 26):
                continue
        ...
        if anchor is not None and endpoint is not None:
            ...
            exponents = np.linspace(20, 26, 121)
    ...
    fig.savefig(HERE/'bch_full_bound_k_scaling_v8.png', dpi=180)
    plt.close(fig)
    ...
    fig.savefig(HERE/'bch_full_bound_state_scaling_v8.png', dpi=180)
    plt.close(fig)
    fig, axes = plt.subplots(2, 2, figsize=(11, 7), layout='constrained')
    for column, block in enumerate((64, 128)):
        top, bottom = axes[:, column]
        weak_labels = []
        for state, color in ((10, '#9467bd'), (12, '#2875aa'), (16, '#ce782e'), (20, '#176c52')):
            ...
            weak_exponents = [r['message_exponent'] for r in selected if r is not None
                              and r['full_margin_bits'] is not None and r['full_margin_bits'] <= 0]
            if weak_exponents:
                weak_labels.append(f"S={state}: log2 K="+', '.join(map(str, weak_exponents)))
            ...
            if not any(math.isfinite(value) for value in margins):
                continue
            top.plot([16, 18, 20, 22, 24], margins, 'o-', color=color, label=f'S={state}')
            bottom.plot([16, 18, 20, 22, 24], losses, 'o-', color=color, label=f'S={state}')
        if weak_labels:
            top.text(.02, .02, 'Full bound remains weak\n'+'; '.join(weak_labels),
                     transform=top.transAxes, fontsize=9, va='bottom',
                     bbox=dict(facecolor='white', edgecolor='none', alpha=.9))
        top.set(title=f'BCH-{block}', ylabel='Full margin (bits)')
        bottom.set(yscale='log', ylabel='Full-bound loss relative to Q1 (bits)', xlabel='log₂ K')
        top.legend()
        for ax in (top, bottom):
            ax.set_xticks([16, 18, 20, 22, 24]); ax.grid(alpha=.2)
    ...
    fig.savefig(HERE/'bch_full_bound_k_state_scaling_v8.png', dpi=180)
    plt.close(fig)
    ...
    summary = dict(rows=len(rows), evidence_counts={label: sum(r['evidence'] == label for r in rows) for label in sorted({r['evidence'] for r in rows})},
                   csv_sha256=sha(path), source_sha256={str(p): sha(p) for p in inputs},
    ...
    (HERE/'bch_engineering_evidence_v8.json').write_text(json.dumps(summary, indent=2)+'\n')
    model = dict(status='ENGINEERING_MODEL_WITH_FIXED_CALIBRATION_CHECKS', checks=model_checks,
                 source_sha256={str(p): sha(p) for p in [Path(__file__), *full_paths]},
    ...
    (HERE/'bch_k_counting_model_v4.json').write_text(json.dumps(model, indent=2)+'\n')
