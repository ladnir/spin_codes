"""Add the exactly enumerated zero weight-14 shell of HQdual."""
import json
from pathlib import Path
from certify_bch_hull_dual_weight14 import build as shell_build
...
FOLDER=ROOT/'generated/oa21_hull_dual16_probe'
PREVIOUS=ROOT/'generated/hull_cut_iterations/round_003/master'


def build():
    shell=shell_build()
    assert shell==json.loads((ROOT/'generated/bch256_hull_dual_weight14.json').read_text())
    assert shell['HQdual_A14_exact']==0 and shell['HQdual_distance_at_least_16']
    model=json.loads((PREVIOUS/'model.json').read_text())
    scales=json.loads((PREVIOUS/'scales.json').read_text())
    kt=kraw_table()
    coeffs={f'{a}_{w}':str(mult*(kt[14][w]+(kt[14][256-w] if w!=128 else 0)))
            for a,mult in (('r',1),('s',255)) for w in range(0,129,4)}
    model['constraints'].append(dict(name='EXACT_HQdual_weight14_zero',coeffs=coeffs,sense='eq',rhs='0'))
    model['metadata']['HQdual14_enumeration']={
        'exact_shell_count':0,'source_sha256':{str(p.relative_to(ROOT)):sha(p) for p in
            (Path(__file__),ROOT/'code/certify_bch_hull_dual_weight14.py',
             ROOT/'generated/bch256_hull_dual_weight14.json',PREVIOUS/'model.json',PREVIOUS/'audit.json')}}
    return model,scales
