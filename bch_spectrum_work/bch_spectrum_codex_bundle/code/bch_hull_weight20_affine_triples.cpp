// Weight-20 variant; frozen weight-18 kernel remains untouched.
#include <array>
#include <bit>
#include <cstdint>
#include <fstream>
...
static bool possible20(Word x,Word y,Word z) {
    ...
    for(unsigned mask=0;mask<8;++mask) {
        ...
        if(24-2*int(u+v+w)==20) return true;
        if(u>=t && v>=t && w>=t && 24-2*int(u+v+w)+4*int(t)==20) return true;
    }
    return false;
}
int main(int argc,char** argv) {
    ...
    for(uint32_t i:reps) {
        ...
        for(uint32_t j=0;j<n;++j) {
            ...
            for(uint32_t k=table[p].head;k!=UINT32_MAX;k=next[k]) {
                if(j>=k) continue;
                ++hits;
                if(possible20(x.w,y.w,records[k].w)) {
    ...
    for(uint32_t i=0;i<n;++i) if(needed[i]) {
        ...
        for(unsigned limb=0;limb<4;++limb) {
            ...
            while(x) {
                if(used>=8) return 7;
                unsigned bit=std::countr_zero(x); members[used++]=field[64*limb+bit]; x&=x-1;
            }
        }
        if(used!=8) return 7;
    ...
    uint64_t affine_checks=0,raw20=0;
    for(const auto t:triples) {
        ...
        for(const auto y:ys) {
            ...
            for(const auto z:zs) {
                ...
                if(weight(word)==20) { ++raw20; seeds.push_back(word); }
            }
        }
    }
    ...
    std::cout<<"{\"queries\":"<<queries<<",\"matching_direction_triples\":"<<hits
        ...
        <<",\"affine_triples_checked\":"<<affine_checks<<",\"raw_weight20_hits\":"<<raw20
        <<",\"distinct_weight20_seeds\":"<<seeds.size()<<"}\n";
}
