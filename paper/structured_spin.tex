...
mixing stage, a sampled binary transvection combines a lazy update with
uniform refresh of each fixed nonzero state. We call this inner
\emph{Independent-Map Transvection} (IMT). Its expansion and feedback maps
are specified separately. The proof combines their certified spectra with
the exact transvection law; it does not assume independent new state bits.
...
\[
  ...
  C:\F_2^t\to\F_2^s,
\]
a setup law for invertible state maps $M_i:\F_2^s\to\F_2^s$, and a joint setup law for
...
\[
  ...
  R:=N/t=be.
\]
The state space is $\F_2^s$. Fix linear maps
\[
  A:\F_2^s\to\F_2^t,
  \qquad
  C:\F_2^t\to\F_2^s.
...
$v_i\gets\{v:\langle u_i,v\rangle=0\}$, including zero, and sets
...
The transpose starts with $r_R=0$ and processes steps in reverse:
\[
