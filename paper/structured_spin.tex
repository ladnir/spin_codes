...
shuffles within those regions randomize the precise locations. A row of weight
$w$ reaches exactly $w$ distinct regions, with at most one bit per region.
This constrains late activation without excluding it. The region shuffles stop the message from
...
\subsection{From asymptotic guarantees to finite parameters}
...
can serve the same interface. The next section instantiates this approach
with a fixed BCH constituent and an RM2Sub inner. The architecture is
unchanged; the finite proof replaces the growing-outer argument with bounds
tailored to the selected constituent.
