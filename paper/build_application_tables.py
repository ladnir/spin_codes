"""Author-side table generator; requires local measurement inputs.

Not part of the core-code artifact. Committed TeX tables build without these
inputs. This script never runs benchmarks.
"""
import argparse
import hashlib
import json
from pathlib import Path
from bolt_communication import communication_estimate

ROOT = Path(__file__).resolve().parent
DATA = ROOT / "data" / "application_results.json"
BOLT_DATA = ROOT / "data" / "bolt_opening_projection.json"
BOLT_COMMIT_DATA = ROOT / "data" / "bolt_x86_commitment.json"
...
def flock_projection(data, case, commitment):
    ...
    total = outer + commitment + shared
    # Retain the opening calibration and refresh only Bolt commitment.
    # The surrounding Flock residual comes from the current SPIN campaign.
    return total


def render(data, bolt, ligerito, bolt_commit):
    rows = []
    ...
    commitment = bolt_commit["cases"]["512"]["commit_ms"]
    total = commitment + bolt["case"]["one_opening_amortized_limit_projection_ms"]
    size = communication_estimate()["estimated_mib"]
    rows.append(r"Bolt-max (amortized)$^{*}$ & "
                + f"${total:.0f}^{{*}}$ & --- & ${size:.1f}^{{*}}$" + r" \\")
    ...
    for log in ("14", "16"):
        ...
        rows.append(f"${2**int(log):,}$ & Bolt (projection) & "
                    f"{flock_projection(data, case, bolt_commit['cases'][str(case['input_mib'])]['commit_ms']):.0f} & --- & ---" + r" \\")
    tables["flock_comparison"] = rows
    digest = hashlib.sha256(DATA.read_bytes()).hexdigest()
    bolt_digest = hashlib.sha256(BOLT_DATA.read_bytes()).hexdigest()
    bolt_commit_digest = hashlib.sha256(BOLT_COMMIT_DATA.read_bytes()).hexdigest()
    communication_digest = hashlib.sha256((ROOT / "bolt_communication.py").read_bytes()
                                          .replace(b"\r\n", b"\n")).hexdigest()
    ...
    outputs = {ROOT / "tables" / (name + ".tex"):
            ...
            + (f"% Bolt projection SHA-256 {bolt_digest}\n"
               if name in ("pcs_standalone", "pcs_opening", "flock_comparison") else "")
            + (f"% Bolt x86 commitment SHA-256 {bolt_commit_digest}\n"
               if name in ("pcs_standalone", "flock_comparison") else "")
            + (f"% Bolt communication model LF SHA-256 {communication_digest}\n"
               if name == "pcs_standalone" else "")
...
def main():
    ...
    if not BOLT_COMMIT_DATA.is_file():
        parser.error("Missing local author input: paper/data/bolt_x86_commitment.json. "
                     "This input is outside the core-code artifact; "
                     "the committed TeX tables can be compiled without it.")
    ...
    bolt_commit = json.loads(BOLT_COMMIT_DATA.read_text(encoding="utf-8"))
    assert bolt_commit["threads"] == 1 and bolt_commit["width"] == 128
    assert bolt_commit["variant"] == "bolt-max-xor-d8" and bolt_commit["hash"] == "sha256"
    assert set(bolt_commit["cases"]) == {"32", "128", "512"}
    ...
    for path, content in render(data, bolt, ligerito, bolt_commit).items():
        if args.check:
