...
def render(data, bolt):
    ...
    outputs = {ROOT / "tables" / (name + ".tex"):
            ...
            + (f"% Bolt projection SHA-256 {bolt_digest}\n" if name == "pcs_opening" else "")
            + "\n".join(rows) + "\n\\bottomrule\n" for name, rows in tables.items()}
    plots = []
    for backend, style in (("spin", "fill=black!65"),
                           ("native-fast100", "fill=black!20")):
        coords = " ".join(f"({i},{data['flock'][log][backend]['total_ms']:.6f})"
                          for i, log in enumerate(("14", "16")))
        plots.append(r"\addplot[" + style + "] coordinates {" + coords + "};")
    coords = " ".join(f"({i},{flock_projection(data, c):.6f})"
                      for i, c in enumerate(bolt["flock_cases"]))
    plots.append(r"\addplot[pattern=north east lines,pattern color=black!70] coordinates {"
                 + coords + "};")
    outputs[ROOT / "figures" / "flock_projection.tex"] = (
        f"% Generated by build_application_tables.py; data SHA-256 {digest}\n"
        f"% Bolt projection SHA-256 {bolt_digest}\n"
        + r"""\begin{tikzpicture}
\begin{axis}[
...
""" + "\n".join(plots) + "\n"
        + r"""\legend{SPIN--Brakedown (measured),Ligerito (measured),Bolt (optimistic projection)}
\end{axis}
\end{tikzpicture}
""")
    return outputs
...
def main():
    ...
    if (args.source or args.bolt_source) and args.check:
    ...
    for path, content in render(data, bolt).items():
        if args.check:
