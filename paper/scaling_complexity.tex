...
\paragraph{Correspondence with the automata formulation.}
The outer block length bounds the data that must be retained to process
one constituent, giving $O(\log N)$ working memory for a blockwise
realization. Relating this directly to the BMS formulation requires
specifying a bit-serial realization, including buffering, termination,
and the treatment of time-dependent transition maps. The structured
interleaver still composes to one permutation between the outer and inner;
the outer's internal permutations operate only within its logarithmic-size
blocks. The construction therefore supplies the relevant memory and
distance regime. A formal statement answering the original question
should make the encoder-model correspondence explicit.
