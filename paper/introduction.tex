...
We instantiate the recursive stage with an Independent-Map Transvection
(IMT) inner.
...
\begin{itemize}
  ...
  about $10$\,ms on one Ryzen 9 7950X thread, approximately $3\times$ faster
  ...
  standalone commitment and one opening take $509$\,ms on one core.
  Commitment is $4.29\times$ faster than the fastest Bolt-max
  configuration~\cite{bolt26}, measured separately at the same input volume.
  Within Flock, replacing
  Ligerito~\cite{ligerito25} with SPIN--Brakedown improves throughput by
  $1.92\times$ and $1.36\times$ at the two measured workloads, with the
