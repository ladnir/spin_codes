...
The application chain begins with a SPIN instantiation of the Brakedown
polynomial commitment scheme and continues with its integration into Flock.
We measure ordinary encoding, standalone commitment and opening, and
complete constraint proving separately. This separates the code's evaluation
cost from the benefit it delivers inside a larger system.
...
\begin{itemize}
  ...
  \item \textbf{A modular structured construction.}\par
  ...
  A fixed BCH-derived $[256,128,\ge38]$ constituent\linebreak and selected $(128,19)$
  ...
  $519.60$\,ms on one core. A separate commitment-only comparison is
  $4.16\times$ faster than the fastest measured Bolt-max configuration
  ...
  \item \textbf{A first-moment framework for structured routes.}\par
  ...
  \item \textbf{Two explanatory SPIN families.}\par
...
~\cite{cryptoeprint:2026/1903}. Structured SPIN adopts that shared-code
...
~\cite{blaze25}. Bolt uses sketched codes to reduce commitment
...
\fi
The guide gives the reproduction entry point, and the map links the theorems,
figures, and timing table to
