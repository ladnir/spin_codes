...
The late-placement summand in the tiny window uses the fixed-tap low-weight
outer law, not only the coarse global envelope.  In particular,
Corollary~\ref{cor:outer-fixedtap-no-weight-one} gives
\begin{equation}
\label{eq:dense-dense-fixedtap-no-weight-one}
A_1^{\mathrm{out,ft}}=0
\end{equation}
deterministically, while Lemma~\ref{lem:outer-fixedtap-banded-lowweight-law}
gives the corresponding band-cluster law for the remaining fixed small weights.
This is the outer input that makes the late-placement contribution summable in
the tiny-weight window.
