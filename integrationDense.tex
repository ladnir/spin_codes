\subsection{Dense+dense integration and asymptotics}
\label{sec:integrations}

This subsection is the theorem-facing integration of the random sliding dense construction:
a systematic dense banded outer together with the random dense recursive inner.
Finite scalar estimates are not used in the proof.  The argument is a direct first-moment calculation:
the outer contributes an expected spectrum envelope, while the inner contributes a low/intermediate-weight contraction
and a linear-weight large-deviation tail.
...
\subsubsection{A baseline linear-distance theorem}
...
\end{equation}
Set \(\beta_{\mathrm{out}}:=z^{-1}\), the outer spectrum constant supplied by
...
Choose \(\xi>0\) small enough that
\[
\lambda_\xi:=(2+\xi)\delta<\rho.
\]
Let
\[
...
\;+\;
\sum_{h=1}^{h_\star}\EE[\enum_{\Cone,h}]
\frac{\binom{L-h+1}{h}}{\binom{n-h+1}{h}}
\;+\;
...
For the late-placement term, choose a constant \(\lambda\) with
\[
\lambda_\xi<\lambda<\rho.
\]
Since \(h\le h_\star=O(\log n)\), for all sufficiently large \(n\),
\[
\frac{\binom{L-h+1}{h}}{\binom{n-h+1}{h}}
\le
\left(\frac{L}{n-h+1}\right)^h
\le
\lambda^h .
\]
Because \(\lambda<\rho<\beta_{\mathrm{out}}^{-1}=z\), the outer generating
bound applies at \(\lambda\), and
\[
\sum_{h=1}^{h_\star}\EE[\enum_{\Cone,h}]
\frac{\binom{L-h+1}{h}}{\binom{n-h+1}{h}}
\le
W_{\mathrm{out}}(\lambda)
\le
C_{\mathrm{out}}(\lambda)\,k\,a(\lambda)^M
=o(1).
\]
Here \(a(\lambda)=(1+\lambda)/2<a(z)\), and the condition
\eqref{eq:dense-c-condition} was imposed at \(z\).
...
\]
\end{proof}
\subsubsection{A linear-weight exponent criterion}
...
Fix constants \(0<\eta_0<\eta_1<1\), \(\delta\in(0,1/2)\),
\(\xi>0\), and \(\theta\in(0,1-\eta_1)\).  For
\(\eta\in[\eta_0,\eta_1]\), let
\[
...
\;\le\;
-\tau
\qquad\text{for every }\eta\in[\eta_0,\eta_1].
\end{equation}
Then the total first-moment contribution from outer weights \(h\ge \eta_0 n\) satisfies
\[
\sum_{h=\lceil \eta_0 n\rceil}^{\lfloor \eta_1 n\rfloor}
...
\begin{proof}
For \(h=\lfloor \eta n\rfloor\) with \(\eta\in[\eta_0,\eta_1]\), Theorem~\ref{thm:outer-dense-linear-exponent} gives
\[
\EE[\enum_{\Cone,h}]
\le
...
\(h\in[\eta_0 n,\eta_1 n]\) preserves the same exponential rate.
\end{proof}
...
The final summation bound is then Theorem~\ref{thm:dense-dense-linear-weight-criterion}
applied with \(\eta_0=\eta_{\mathrm{crit}}\) and \(\eta_1=0.99\).
The only computer-assisted ingredient is this explicit one-variable exponent
check; a final formalization can replace the sampled-grid check by an interval
certificate for the same formulas.
\end{proof}
...
\(\rho=1/4\), and for instance \(\xi_{\mathrm{tiny}}=1/4\).  Then
\[
(2+\xi_{\mathrm{tiny}})0.109<\frac14,
\]
so the late-placement summation in that proof is valid.  The remaining
conditions are
\[
\frac12\log_2 3<1,
\qquad
2\cdot0.109<\frac14<\frac13,
\qquad
\gamma>2=1+\frac12\log_2 4,
\]
and \(c>1/\log_2(3/2)\), so \(S_{\mathrm{tiny}}=o(1)\).

For \(S_{\mathrm{low}}\), use the global geometric outer envelope from
Corollary~\ref{lem:outer-dense-spectrum} at \(z=1/3\):
\[
\EE[\enum_{\Cone,h}]\le 3^h .
\]
Corollary~\ref{cor:dense-inner-constant-geometric} applies with \(\rho=1/4\), \(\kappa=1/2\), and \(\gamma>2\),
...
S_{\mathrm{low}}
\le
3\sum_{h=h_0+1}^{\lfloor \eta_{\mathrm{crit}}n\rfloor}
\left(\frac34\right)^h.
\]
This is a geometric tail beginning at \(h_0=\tfrac12\log_2 n+O(1)\), hence
\[
S_{\mathrm{low}}
\le
n^{-\frac12\log_2(4/3)+o(1)}
=
o(1).
\]
...
If \(h=\lfloor \eta n\rfloor<n\) with \(\eta\in[0.99,1)\), then Corollary~\ref{cor:outer-dense-highweight-exponent} gives
\[
\Phi_{\mathrm{out}}^{\mathrm{sys}}(\eta)=H_2(\eta)-\frac12
\le
H_2(0.99)-\frac12
...
n^{O(1)}2^{-0.41n},
\]
uniformly over \(h\in[0.99n,n-1]\), and so
\[
\sum_{h=\lceil0.99n\rceil}^{n-1}\EE[\enum_{\Cone,h}]\,p_h(0.109)
\le
n^{O(1)}2^{-0.41n}
=
o(1).
\]
The endpoint \(h=n\) is harmless as well.  A codeword of weight \(n=2k\)
forces the systematic input to be \(1^k\); for this unique input, all but the
first parity-one constraint are independent fair constraints, so
\[
\EE[\enum_{\Cone,n}]\le 2^{-(k-1)}=2^{-n/2+1}=o(1).
\]
Thus \(S_{\mathrm{top}}=o(1)\).
