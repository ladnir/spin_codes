...
The framework yields two main results in the current paper.  First, the random sliding dense outer paired with the dense
recursive inner gives a clean asymptotic linear-distance theorem, including the concrete \(0.109\) relative-distance
checkpoint.  Second, the structured local-code construction gives a finite certificate: \(RM(4,9)\) direct-sum outer plus
the full-split EBCH inner at \(N=2^{21}\), \(\delta=.09\), with first moment at most \(2^{-37.278528}\).
...
Section~\ref{sec:outer-dense-logmem} proves the random sliding dense outer bounds.
Section~\ref{sec:inner-accumulator} records the accumulator warmup, and Section~\ref{sec:inner-dense} proves the scalar dense
recursive inner interface.  Section~\ref{sec:integrations} gives the analytic dense+dense integration theorem.
