...
Section~\ref{sec:inner-accumulator} records the accumulator warmup.  Section~\ref{sec:outer-dense-logmem} proves the
random sliding dense outer bounds, Section~\ref{sec:inner-dense} proves the scalar dense recursive inner interface, and
Section~\ref{sec:integrations} gives the analytic dense+dense integration theorem.
Section~\ref{sec:structured-local-code} gives the local-code outer interface,
Section~\ref{sec:fullsplit-local-code-inner} gives the full-split inner
interface, Section~\ref{sec:rm-ebch-finite-certificate} states the RM/EBCH
finite checkpoint, and Section~\ref{sec:local-code-projections} records BCH
and other local-code projection paths.
