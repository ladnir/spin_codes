...
\end{array}
\]
The worst sampled value, \(-67.64\), inflates the termination atom only from \(-64.004574\) to \(-63.892649\).  Rerunning
the \(h=500,T=13949\) episode-gap diagnostic with this inflated atom changes the inner exponent from \(-673.38\) to
\(-673.27\), and the near \(h=32,T=5949\) ridge is unchanged because it is dominated by the no-termination survival
branch.  Thus exact input/state cancellation can be folded into the same skipped-gap ledger, at least for the current
\(h\le500\) finite prefix, by replacing \(p_0\) with
\[
p_{\mathrm{term}}:=p_0+\eta,\qquad
\eta=\sup_{H,T}\sum_{q>0}\Pr[Q=q]\frac{\binom{b(T-1)}{H-q}}{\binom{bT}{H}}.
\]
The cancellation diagnostic is
\[
\texttt{python scripts/analyze\_fullsplit\_exact\_cancellation.py}.
\]
...
\end{array}
\]
The peak row is still \(h=32,r=1\), gap \(1\text{--}4000\), with inner exponent \(-20.011735\) and full term
\(-37.385767\).  This is not yet a theorem, because the interpolation envelope between sampled \(H\)-knots must be
justified or replaced by a grid-plus-monotonicity certificate.  It does show that the prefix no longer has a hidden
intermediate-\(h\) ridge once the episode-gap correction is included.  The envelope summation helper is
\[
...
\[
\log_2 p_{\rm term}=-63.892649.
\]
Using full integer \(H=0,\ldots,499\) CSVs, with extrapolation disallowed in the envelope summation, removes the
interpolation issue from the finite prefix.  The current \(e\le8\) ledger gives the total prefix value
\[
-34.767174,
\]
...
8001\text{--}12000:\ -85.867481.
\]
There is one remaining \(T\)-dependence caveat in this exact-grid prefix: the summation wrapper indexes the exact
inner CSV by \(H\), while a placement bucket contains a range of residual lengths \(T\).  As a first audit guard, we
computed right-endpoint grids with the same inflated termination atom and compared the exact \(e\le8\) total against
the left-endpoint grids for every \(H=0,\ldots,499\):
\[
\begin{array}{c|c|c|c}
\text{gap bucket} & \max_H(\log_2 p_{\rm right}-\log_2 p_{\rm left})
& H_{\rm max} & \text{largest single-column wiggle}\\
\hline
1\text{--}4000 & -43.632069 & 1 & +0.002016\ (e=8,H=7)\\
4001\text{--}8000 & 0 & 0 & +0.000860\ (e=8,H=7)\\
8001\text{--}12000 & 0 & 0 & +0.000477\ (e=8,H=7).
\end{array}
\]
Thus the bucket right endpoints do not exceed the left-endpoint exact totals; the equality cases are the \(H=0\)
pure-termination rows.  The positive entries in the last column are individual fixed-\(e\) terms, not the log-summed
total used by the ledger.  This endpoint check is performed by
\[
\texttt{python scripts/check\_fullsplit\_exact\_grid\_endpoints.py}.
\]
It is not yet the desired analytic monotonicity proof for all interior \(T\), but it removes the immediate
left-versus-right endpoint ambiguity in the finite prefix.
...
=-269.335258.
\]
The peak of this residual is again the far bucket at \(h=500,r=1\), but it is now far below the explicit \(e\le8\)
ledger.  Combining the explicit \(e\le8\) ledger with this \(e\ge9\) residual leaves the current finite-prefix bound
\[
\log_2 \mu_{\le500}^{\rm fullsplit}\le -34.767174.
\]
...
If the selected gaps have total length \(s\), then \(T-s\) live BCH split branches remain to be charged.  For fixed
\(\lambda\),
\[
...
\(p_{\rm term}=2^{-63.892649}\), \(N=2^{21}\), \(b=64\), and \(\delta=.09\), optimizing a coarse
...
\eqref{eq:fullsplit-e1-cauchy-envelope} together with the same-\(\lambda\) multiplier
\eqref{eq:fullsplit-e-tail-ht-multiplier}.  Thus this mode is an all-episode upper bound, not merely an \(e\le1\)
diagnostic.
...
independent binomial evaluations.  The matching inner endpoint must be handled with some care: if the left endpoint
\(T_{\min}\) has \(H>bT_{\min}\), then early gaps in the bucket are impossible but later gaps are not.  The corrected
working rule is therefore to use the earliest feasible block count
\[
...
\hline
2001\text{--}7858 & (.02,.01) & -588.081877\\
7859\text{--}20550 & (.05,.03) & -2657.429602\\
20551\text{--}75000 & (.2,.1) & -6027.058821\\
75001\text{--}250000 & (.5,.3) & -12846.353611\\
250001\text{--}350000 & (1.2,.5) & -28478.125315\\
350001\text{--}400000 & (1.2,1) & -81557.079312\\
400001\text{--}450000 & (1.2,1) & -91423.257760\\
450001\text{--}550000 & (1.2,1) & -82083.446102\\
550001\text{--}650000 & (1.2,1) & -1519.978235\\
650001\text{--}725000 & (1.2,2) & -116329.618720\\
725001\text{--}750000 & (1.2,3) & -169889.387695\\
750001\text{--}850000 & (1.2,3) & -134525.068420\\
850001\text{--}950000 & (1.2,3) & -117536.328304\\
950001\text{--}1050000 & (1.2,3) & -285054.035442\\
1050001\text{--}1148736 & (1.2,3) & -562862.201412 .
\end{array}
\]
...
\]
It combines the explicit \(32\le h\le500\), \(e\le8\) CSV ledger, the crude \(e\ge9\) prefix residual, the
all-episode \(501\le h\le2000\) wrapper row, and the fixed-pole interval table above.  The reported combined finite
late-window contribution is
\[
\log_2\mu_{\rm finite}^{\rm fullsplit}\le -34.767174,
\]
dominated by the tiny-prefix row \(h=32,r=1\) in the near bucket; the \(e\ge9\) prefix residual is already
\(-269.335258\) bits and the entire \(h\ge501\) ledger is below \(-182.259739\) bits.
