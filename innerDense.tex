...
one-live-branch termination atom in the following sense: after exposing any earlier blocks, earlier scramblers and
splits, the current state weight, and the remaining input-placement counts, the probability of the charged termination
event at the next selected opportunity is at most \(p_{\rm term}\).  The charged event may be the split atom \(q'=0\), or
an exact input/state cancellation atom bundled into the same opportunity.

Then, for any fixed placement of the later input blocks and any fixed set \({\cal S}\) of \(e\) selected gaps in the
episode decomposition,
\[
\Pr[\text{all selected opportunities in }{\cal S}\text{ terminate}\mid\text{placement}]
\le
p_{\rm term}^{e}.
\]
\end{lemma}
...
controls the \(q'=0\) atom independently of the past.  Conditional on a state support of size \(q\), the next input block
support is uniform among supports with the exposed occupancy, giving the exact-cancellation atom.  The numerical
finite-prefix and global values of \(p_{\rm term}\) are the corresponding union bounds for these two local atoms.
\end{proof}
...
\texttt{python scripts/verify\_fullsplit\_finite\_ledger.py}.
\]
It first checks the tracked interior-\(T\) audit artifact for the \(32\le h\le500\), \(e\le8\) prefix, then combines
that explicit CSV ledger with the crude \(e\ge9\) prefix residual, the all-episode \(501\le h\le2000\) wrapper row,
and the fixed-pole interval table above.  The reported combined finite late-window contribution is
\[
