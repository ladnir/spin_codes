...
\le 2^{-41.113442},
\]
using exact RM direct-sum outer coefficients through \(h=500\).  The window ledger itself is
\[
...
through \(h=N/2\).  The remaining first-active coverage gap is the post-prefix part of the ultra-late cap beyond
\(h=500\).
...
\]
and has \(114\) positive supported weights in \(1\le h\le500\).  Replacing the smoothed outer rows in the audited
...
the checked early endpoint table for \(T>17948\) through \(h=75000\).  The third row is the vectorized endpoint
extension for \(75001\le h\le350000\), and the fourth row is the paired-\(T\) extension for
\(350001\le h\le N/2\).  The fifth row is the fixed-pole interval table through the feasible far-bucket cutoff
\(h=1148736\).  The sixth row is the complement-side high-density table.  The last two row families overlap on
\(1048577\le h\le1148736\); the ledger deliberately keeps this harmless overcount as a union bound.  Therefore the
combined contribution is bounded by
\[
\begin{aligned}
&\log_2\!\left(2^{-182.259739}+2^{-380.829185}+2^{-1056.187188}+2^{-98386.449256}\right.\\
&\qquad\left.+2^{-586.597103}+2^{-35150.097366}\right)\le -182.259739.
\end{aligned}
\]
...
early endpoint rows through \(h=75000\), the accelerated early rows through \(h=N/2\), and the fixed-pole interval
tables above, including the complement-side high-density manifest.  Reweighting the \(h\le500\) pieces by exact
