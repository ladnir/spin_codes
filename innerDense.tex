...
\]
The verifier flag \(\texttt{--print-late-postprefix-commands}\) prints the regeneration commands; the row helper is
\(\texttt{certify\_fullsplit\_late\_prefix\_interval.py}\).
...
\(350001\le h\le N/2\).  The sixth row is the fixed-pole interval table through the feasible far-bucket cutoff
\(h=1148736\).  The seventh row is the complement-side high-density table.  The last two row families overlap on
