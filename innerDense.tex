...
inner CSV by \(H\), while a placement bucket contains a range of residual lengths \(T\).  For the finite
\(N=2^{21}\) certificate this is now checked directly.  The skipped-gap suffix
\[
\sum_{L=x}^{T}\binom{T-L+e-1}{e-1}\binom{L-e}{x-e}\,S(L)
\]
is swept by a short cumulative recurrence, so all interior residual lengths can be audited without regenerating a
CSV at every \(T\).  Running over every \(H=0,\ldots,499\) and every \(T\) in the three 4000-block placement buckets
gives:
\[
\begin{array}{c|c|c|c}
\text{checked set} & \#\text{rows} & \max(\log_2 p_T-\log_2 p_{\rm left}) & T_{\rm max}\\
\hline
H=0,\ldots,499;\ \text{all three buckets} & 1500 & 2.2\cdot 10^{-9} & \text{left endpoint}.
\end{array}
\]
The small positive value is floating roundoff in reproducing the left endpoint, not an interior increase.  The audit is
performed by
\[
\texttt{scripts/check\_fullsplit\_exact\_grid\_interior.py}.
\]
This is still a finite certificate rather than an analytic monotonicity theorem, but it removes the dominant
tiny-prefix \(T\)-reuse assumption from the current ledger.
