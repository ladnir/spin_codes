...
\hline
\text{ultra-late} & 0\le T<5949 & \text{placement cap checked for }h\le500\\
\text{audited window} & 5949\le T\le17948 & \text{episode ledger checked}\\
\text{early} & T>17948 & \text{episode ledger checked for }h\le500
\end{array}
\]
...
\le 2^{-40.115431}.
\]
The window ledger itself remains
\[
\log_2\mu_{\rm window}\le -34.767174,
\]
and the union of this checked window with the checked ultra-late prefix has
\[
\log_2\mu_{\rm ultra\mbox{-}late\ prefix\ +\ window}\le -34.732187.
\]
...
\[
\log_2\mu_{32\le h\le500}^{\rm all\ first\ active\ positions}\le -34.732187,
\]
still dominated by the audited window and ultra-late prefix.  The proof debt is therefore no longer the boundary slice
\(T\approx5949\), nor the small-weight early slice; it is the remaining first-active coverage for \(h>500\), together
with the post-prefix part of the ultra-late cap.
...
\(-37.385767\).  The important refinement is that the full prefix is not a shapeless table effect.  It is a narrow
near-boundary ridge:
\[
\begin{array}{c|c}
...
\end{array}
\]
The total prefix value is \(2.618593\) bits above the single peak row.  Along the dominant \(r=1\), first-gap ridge,
the largest adjacent ratio is \(0.8526993\) from \(h=32\) to \(33\), and after that it is at most \(0.8337949\) in the
audited range.  Thus the next theorem target is an analytic geometric-ridge bound for the \(h\)-sum, not a collection
of isolated fixed-\(h\) estimates.  The finite version already has this form: replacing the exact ridge by the infinite
geometric bound
\[
...
\le .303594,
\]
with the maximum at \(h=32\).  The exact peak is \(0.303593738594\).  The naive endpoint bound
\[
...
\(S_H\) rather than endpoint domination.  Thus the analytic replacement should prove the outer slope bound and this
averaged placement slope bound, or a direct product bound, rather than re-summing all \(469\) ridge rows.
The standard verifier
\[
\texttt{python scripts/verify\_fullsplit\_finite\_ledger.py}
\]
also checks this ratio skeleton directly, and the placement subcheck can be run alone as
\[
...
left-endpoint reproduction error is at most \(5\cdot10^{-6}\).  The current artifact has the numerical maxima stated
above.
\end{proof}
...
\]
The tight part of this inequality is exactly the near-boundary geometric ridge described above; the middle/far buckets
and the crude \(e\ge9\) tail are already many dozens of bits lower.
...
\(e\le16\), the crude early \(e\ge17\) residual, the all-episode \(501\le h\le2000\) wrapper row, and the fixed-pole
interval tables above, including the complement-side high-density manifest.  The reported combined finite late-window
...
\(-269.335258\) bits, the early \(h\le500\) contribution is \(-85.403338\) bits, and the entire \(h\ge501\)
late-window ledger is below \(-182.259739\) bits.  Including the checked ultra-late placement prefix, the all-position
\(32\le h\le500\) value is
\[
\log_2\mu_{32\le h\le500}^{\rm all\ first\ active\ positions}\le -34.732187.
\]
