...
factor;
\item exact outer-only comparisons also suggest caution: in the dominant tiny-weight window the current closed-form
outer count appears to sit a nearly constant few bits below the exact systematic-outer tables, so even the
pairwise-calibrated run-mixture estimate should presently be read as somewhat optimistic until that normalization
issue is either explained or corrected;
...
crosses below \(1\) near \(c=4\), the pairwise-calibrated run-mixture model crosses near
\(c=5\), and the more conservative single-run model crosses only near \(c=6\), so the present evidence should be read
...
The outer small-weight formula \eqref{eq:dense-dense-working-outer-small} is already explicit and usable, but the
current exact outer-only comparisons indicate that its normalization is still a few bits low in precisely the
window that dominates the first moment.
The remaining gap is therefore split between that outer normalization issue and the inner side:
