...
\begin{lemma}[Bounded-\(r\) low-slot geometric ratio]
\label{lem:bounded-r-low-slot-ratio}
Fix \(R\ge1\).  Under the same outer generating-function bound \(A_h^{\mathrm{out}}\le W(z)z^{-h}\), consider any
fixed \(1\le r\le R\) global-cover subpiece.  Its support factor has the form
\[
\frac{\binom{M}{h-c}}{\binom{N}{h}},
\qquad
c\in\{2r,2r+1\},
...
\begin{lemma}[Bounded-\(r\) high-slot suppression below the linear splice]
\label{lem:bounded-r-high-slot-suppression}
Fix constants \(R\ge1\), \(0<\delta<\theta/2<1/2\), \(z\in(0,1)\), and \(0<\eta<1\).  For any fixed
\(1\le r\le R\), restrict the global-cover subpieces to support-slot length \(M=xN\ge\theta N\).  Then their total
...
For the current certificate \(R=8\) and \(H=2000\).  With the same \(\theta=0.35\) and \(z=0.370884\),
\[
\log_2 q_{H,8,\theta,z}\approx -0.072,
\]
so the low-slot residual remains geometric for every \(1\le r\le8\).  The high-slot exponent is unchanged from the
\(r=1\) calculation and remains positive for the bridge splice choices above.  Thus the finite-prefix certificate now
covers the bounded-\(r\) episode-cover pieces used by the evaluator.  The remaining proof task is to add a separate
tail over large \(r\), using the fixed-tap factor \(2^{-r(\sigma-1)}\) and the interval-count entropy.
