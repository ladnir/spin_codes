...
At the time of writing, the strongest finite-\(n\) working estimate is obtained by an
\emph{adaptive exact tiny-weight sum}: we evaluate the exact outer small-\(h\) count together with the exact
run-tail inner bound term-by-term until the contribution sequence has clearly fallen well below its peak.
Beyond that point, the original geometric residual can be replaced by a local continuation model based on the observed
decay of the exact tiny-weight terms.  Empirically, this makes very little difference for the candidate transition:
the dominant mass is already concentrated at very small \(h\), typically around \(h\approx 5\text{--}10\) in the
\(\sigma=\lceil \log_2 k\rceil + c\) regime of interest.

Under this refined model, the current picture for the construction tracked by the enumerator is:
\begin{itemize}
...
\item exact inner-only comparisons now suggest caution: at the very smallest weights, the present run-tail inner model
can still be optimistic, so a more conservative competing heuristic is to anchor the tiny-weight law at the
single-run probability \(p_1(\delta)\) and then correct the multi-run behavior by an explicit pairwise interaction
factor;
\item after replacing the old convolutional proxy by the actual systematic banded small-weight law used in the
enumerator, the outer mismatch becomes much smaller; the dominant uncertainty is therefore pushed back onto the inner
tiny/intermediate-weight law rather than the outer count itself;
...
\item more specifically, the current pairwise-calibrated run-mixture model treats an \(s\)-run input as having
effective tail probability
\[
q_s \approx q_1^s \exp(-\gamma(q_1)\tbinom{s}{2}),
\]
where \(q_1\approx p_1(\delta)\approx 2\delta\) and the exact inner-only tables suggest a simple linear interaction law
\(\gamma(q_1)\approx a+bq_1\) over the regime \(q_1\in[0.17,0.25]\);
\item at the concrete point \(k=2^{20}\), \(\delta=0.12\), and \(n=2k+\sigma\), the current optimistic refined model
crosses below \(1\) near \(c=5\), the pairwise-calibrated run-mixture model crosses near
\(c=6\), and the more conservative single-run model crosses only near \(c=8\), so the present evidence should be read
as a bracket rather than a single claimed threshold.
\item this bracket is not a single-point artifact. In the current finite-length sweep over
\(k=2^{10},2^{12},\dots,2^{20}\) at \(\delta=0.12\), the first offsets with estimated first moment below \(1\) are:
\[
...
\end{array}
\]
So the present evidence is consistent with a transition region that drifts upward slowly with \(k\), with the
pairwise-calibrated lane currently centered around \(c\approx 6\) by the time \(k\) reaches \(2^{20}\).
\item overlap checks against the exact enumerator, using the observed threshold \(\delta=h_{\mathrm{thr}}/n\) and the exact
first-moment value at that threshold, now show that the pairwise-calibrated and single-run lanes are much closer to the
true finite-length behavior than the earlier convolutional proxy, but they still tend to be optimistic by a few bits on
average, with the miss becoming larger as \(\sigma\) grows. So even the present \(c\approx 6\) center should still be
read as a working estimate rather than a certification.
\item a useful theorem-shaped refinement is to replace the least-squares pairwise damping fit by a conservative affine
lower envelope \(\gamma_{\mathrm{safe}}(q_1)\) for the minimum observed pairwise exponent across the current exact
inner-only tables. Empirically this moves the central lane only slightly: at \(k=2^{20}\) and \(\delta=0.12\), the
pairwise-calibrated model crosses below \(1\) near \(c=6\), and the safe-pairwise version does the same. So the
current evidence suggests that a proof built around a pairwise run-mixture upper bound may lose only a small constant
amount relative to the present central heuristic.
\item accordingly, the current working language is to treat these three lanes as
\emph{optimistic}, \emph{central}, and \emph{pessimistic} estimates rather than as competing theorem claims.
\end{itemize}
...
working estimate to a finite-\(n\) bound, together with a cleaner interpolation from that regime to the
