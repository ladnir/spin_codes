...
Berry--Esseen binomial-tail upper bound near the central suffix edge, the current blocked finite-\(n\) certificate gives
\[
\begin{array}{c|c|c|c}
...
Thus the current conservative certificate crosses near
\[
\sigma\approx \lceil\log_2 k\rceil+5
\]
for the fixed \(k=2^{20}\), \(\delta=0.106\) test point.  The sharper survival-only bridge crosses near offset \(0\),
so the remaining proof slack is now visible: it is concentrated in the tiny \(h\approx 5\text{--}10\) late-placement
base and in the remaining conservative episode-cover overcount, not in a medium-weight tail obstruction.
