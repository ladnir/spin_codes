...
\paragraph{Current summability status of the paired-cover theorem.}
The theorem-safe version of the finite-\(n\) check replaces the sampled search over \(T\) by a blocked upper sum: on
each block \([T_0,T_1]\), the decreasing interval-placement factor is evaluated at \(T_0\), the increasing support
factor at \(T_1\), the binomial lower-tail factor at \(T_0\), and the block length is paid explicitly. This is much
more pessimistic than the fitted diagnostic, but it is the right kind of object for a proof.

At \(k=2^{20}\), \(N=2k\), \(\delta=0.106\), and \(L=\lceil 2.1\delta N\rceil\), this blocked paired-cover bound for
the low/intermediate prefix \(h\le64\) is still slightly above one at
\(\sigma=\lceil\log_2 k\rceil+11=31\), but is below one at
\(\sigma=\lceil\log_2 k\rceil+12=32\). The dominant failing shape just below this point is not an adjacency remainder:
it is the \(r=1\) event where one long early episode of live length just below \(2\delta N\) swallows many input-one
positions and pays only one fixed-tap termination. Thus the present proved lane supports the fixed-tap strategy with a
larger offset, while improving the offset back toward the heuristic \(\sigma=28\) range requires a sharper treatment of
this one-long-episode cluster.

For reference, the current blocked-cover prefix certificates are:
\[
\begin{array}{c|c|c|c|c}
k & \sigma & \sigma-\lceil\log_2 k\rceil & \delta & \log_2 \mu_{\le64}^{\mathrm{FCIB}}\\
\hline
2^{18} & 30 & 12 & 0.106 & -1.934\\
2^{20} & 31 & 11 & 0.106 & \phantom{-}0.026\\
2^{20} & 32 & 12 & 0.106 & -1.684
\end{array}
\]
Here \(\mu_{\le64}^{\mathrm{FCIB}}\) denotes the fixed-tap outer first-moment contribution from the paired-cover
bound on \(1\le h\le64\), together with the already-negligible linear and top-weight estimates used by the evaluator.
These are finite-\(n\) certificates for the checked prefix, not yet an all-\(h\) asymptotic theorem. To turn the
\(C=12\) evidence into a complete fixed-\(k\) proof one still has to write the remaining medium-weight bridge and the
large-weight exponent estimates as a single auditable first-moment certificate.

A subsequent cap extension shows that the \(h\le64\) prefix was affected by the artificial boundary. At the same
parameters \(k=2^{20}\), \(\delta=0.106\), and \(\sigma=32\), increasing the blocked-cover prefix to \(h\le80\) gives
\[
\log_2 \mu_{\le80}^{\mathrm{FCIB}}=1.282,
\]
with the dominant contribution again occurring at the cap \(h=80\). Thus the \(C=12\) line should be read only as a
tiny/intermediate-prefix sanity check, not as a credible fixed-\(k\) certificate. The current theorem-safe paired-cover
bound still has an unresolved medium-weight burst problem: as \(h\) grows, a single live interval of length
\(\approx 2\delta N\) can absorb more support entropy than the present \(r=1\) charge pays for.
