...
const char* Spin::name() const noexcept {
    if(mOuter==Outer::Bch128x32) return "t128_s19_asymmetric_greedy3_2_r1";
...
template<class Map,bool Packed,bool Quarter> void Spin::run(const block* in,block* out,Workspace& w) const {
    ...
    profile.inner+=millis(profileStart);++profile.calls;
    ...
    for(std::size_t base=0;base<n;base+=tileSize) {
        ...
        profile.scatter+=millis(scatterStart);
        ...
        profile.outer+=millis(outerStart);
    }
}
