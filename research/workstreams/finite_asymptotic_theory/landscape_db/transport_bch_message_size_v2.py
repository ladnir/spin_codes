...
With --replace-weak, a preceding reference is archived and retained if
the candidate does not improve its full margin.
"""
import argparse
import copy
import json
...
def main():
    ...
    parser.add_argument('--replace-weak', action='store_true', help='Try to strengthen an existing weak reference.')
    ...
    for exponent in dict.fromkeys(args.exponent):
        ...
        if full_path.exists():
            ...
            if not args.replace_weak or existing['full_margin_bits'] > 0:
                print(f'{tag}: existing verified reference retained', flush=True)
                continue
            preceding = full_path.read_bytes()
        maximum = min(length, source['occupation_max']); minimum = maximum+1
        directory = HERE/f'bch_sparse_k_transport_v2_{tag}_q5_{maximum}'
        ...
        def run(script, extra, stage, with_geometry=True):
            ...
            with (HERE/f'bch_k_transport_v2_{tag}_{stage}.log').open('w') as output:
        ...
        if preceding is not None:
            ...
            archive = history/f'{full_path.stem}_{study.sha(full_path)}.json'
            if not archive.exists():
                archive.write_bytes(preceding)
        run('verify_bch_full_reference_v5.py', extra, 'verify')
        final = json.loads(full_path.read_text())
        if preceding is not None and final['full_margin_bits'] < existing['full_margin_bits']:
            candidate = history/f'{full_path.stem}_{study.sha(full_path)}.json'
            if not candidate.exists():
                candidate.write_bytes(full_path.read_bytes())
            full_path.write_bytes(preceding)
            print(f'{tag}: retained the stronger preceding reference', flush=True)
        else:
