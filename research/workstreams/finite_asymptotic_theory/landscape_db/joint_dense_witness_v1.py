...
class JointRefiner(fast.FastRefiner):
    def objective(self, corners, penalty, anchor, probabilities):
        ...
        def evaluate(coordinates):
            ...
            for j in range(1, len(p)):
                if p[j] == 1.:
                    ...
                    active = int(np.argmax(candidates))
                    costs[j] = candidates[active]
                    derivatives[j] = self.block*p[j]-weights[j-1][active]
    ...
    def refine(self, box):
        ...
        result = minimize(objective, initial, jac=True, method='L-BFGS-B',
                          ...
                          options={'maxiter': 200, 'ftol': 1e-14, 'gtol': 1e-10, 'maxls': 40})
