...
def check_intervals(intervals, length, dense_minimum):
    if (not intervals or intervals[0][0] != 5 or intervals[-1][1] != length
            or any(lo > hi for lo, hi in intervals)
            or any(a[1]+1 != b[0] for a, b in zip(intervals, intervals[1:]))
            or (dense_minimum <= length and intervals[-1] != (dense_minimum, length))):
...
def main():
    ...
    parser.add_argument('--sparse-checkpoint', type=Path,
                        help='Optional authenticated Q2..4 component refinement.')
    parser.add_argument('--sparse-cover', type=Path, action='append',
                        help='Explicit sparse interval receipts, ordered from Q5 upward.')
    parser.add_argument('--dense-cover', type=Path,
                        help='Explicit complete dense interval, including transported covers.')
    ...
    if length < 5 or length % t or args.dense_minimum < 5:
        raise ValueError('Expected L>=5, complete epochs, and a dense cutoff >=5')
    has_dense = args.dense_minimum <= length
    if args.dense_cover and not has_dense:
        raise ValueError('Explicit dense cover requested for an empty dense interval')
    ...
    if args.sparse_checkpoint:
        baseline = sparse
        checkpoint = args.sparse_checkpoint.resolve()
        sparse = json.loads(checkpoint.read_text())
        if sparse['status'] != 'BINARY64_SPARSE_COMPONENT_REFINEMENT':
            raise ValueError('unsupported sparse checkpoint')
        if sparse['summary']['q1_margin_bits'] != baseline['summary']['q1_margin_bits']:
            raise ValueError('refinement changed Q1')
        refined_dependencies = sparse['source_sha256']
    ...
    with mp.workdps(90):
        for q in (2, 3, 4):
            detail = sparse['occupations'][str(q)]
            multiplicities = transfer.composition.log_multiplicities(np.array(detail['compositions']))
            index = int(np.argmax(np.array(detail['component_log_upper'])+multiplicities))
            tilt, shift = detail['witnesses'][index]
            value = mp_reference.component(t, s, ac, config['kernel_counts'], length, block,
                                           counts, detail['bands'], detail['compositions'][index], tilt, shift)
            error = abs(float(value)-detail['component_log_upper'][index])
            if error > 2e-7:
                raise ArithmeticError('90-digit sparse composition replay failed')
            sparse_mp_checks.append(dict(occupation=q, component=index, absolute_log_error=error))
    combined_maximum = min(length, 256, args.dense_minimum-1)
    ...
    if args.dense_minimum > 257 and length >= 257:
        paths.append(HERE/f'bch_sparse_tail_v3_b{block}_t{t}_s{s}_e{exponent}_q257_{min(length, args.dense_minimum-1)}/cover.json')
    ...
    if args.dense_cover:
        dense_path = args.dense_cover.resolve()
    if has_dense:
        paths.append(dense_path)
    payloads = [json.loads(path.read_text()) for path in paths]
    dependencies = dict(sparse_receipt['source_sha256'])
    for source_map in [refined_dependencies, *(data['source_sha256'] for data in payloads)]:
    ...
    check_intervals(intervals, length, args.dense_minimum)
    coefficient_checks = []
    for data in (payloads[:-1] if has_dense else payloads):
    ...
    result = dict(status='VERIFIED_BINARY64_FULL_REFERENCE', block_bits=block, step_bits=t, state_bits=s,
                  ...
                  coefficient_checks=coefficient_checks, sparse_90_digit_checks=sparse_mp_checks,
                  dense_cover=coverage, dense_90_digit_checks=mp_checks,
                  authenticated_dependencies=len(dependencies),
                  source_sha256={**{str(study.ROOT/p): digest for p, digest in dependencies.items()},
                                 **{str(p): study.sha(p) for p in [Path(__file__), Path(lazy.__file__), Path(density.__file__), Path(mp_reference.__file__), checkpoint, *paths]}},
