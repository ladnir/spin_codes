...
def main():
    ...
    model = previous.CoverRefiner(counts, block, t, s, ac, config['kernel_counts'], length, raw['bands'])
    directory = HERE/f'bch_dense_v4_b{block}_t{t}_s{s}_e{e}_q{args.minimum}'
    ...
    for index, root in enumerate(raw['selected_boxes']):
        path = old_directory/f'root_{index}.json'
        selected = [root]
        if path.exists():
    ...
    for path in (Path(__file__), Path(previous.__file__), Path(checks.__file__), Path(previous.base.__file__),
