...
import joint_dense_witness_v2 as fast
...
def main():
    ...
    directory = HERE/f'bch_dense_v8_b{block}_t{t}_s{s}_e{e}_q{args.minimum}'
    directory.mkdir(exist_ok=True)
    old_directory = None
    resume = directory/'checkpoint.json'
    if not resume.exists():
        resume = HERE/f'bch_dense_v7_b{block}_t{t}_s{s}_e{e}_q{args.minimum}/checkpoint.json'
    if resume.exists():
