...
def main():
    ...
    model = fast.FastCoverRefiner(counts, block, t, s, ac, config['kernel_counts'], length, raw['bands'])
    directory = HERE/f'bch_dense_v5_b{block}_t{t}_s{s}_e{e}_q{args.minimum}'
    directory.mkdir(exist_ok=True)
    old_directory = HERE/f'bch_dense_v3_b{block}_t{t}_s{s}_e{e}_q{args.minimum}'
    resume = directory/'checkpoint.json'
    if not resume.exists():
        resume = HERE/f'bch_dense_v4_b{block}_t{t}_s{s}_e{e}_q{args.minimum}/checkpoint.json'
    if resume.exists():
    ...
    for path in (Path(__file__), Path(fast.__file__), Path(previous.__file__), Path(checks.__file__), Path(previous.base.__file__),
