...
def main():
    ...
    directory = HERE/f'bch_sparse_tail_v2_b{block}_t{t}_s{s}_e{exponent}_q{args.minimum}_{maximum}'
    directory.mkdir(exist_ok=True)
    for path in (Path(__file__), Path(transfer.__file__), Path(positive.__file__)):
