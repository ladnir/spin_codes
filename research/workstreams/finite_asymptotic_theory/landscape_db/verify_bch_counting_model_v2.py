...
def main():
    path = HERE/'bch_k_counting_model_v4.json'
    ...
    if not {(b, e) for b in (64, 128) for e in range(21, 27)} <= {
            (row['block_bits'], row['comparison_exponent']) for row in checks}:
        raise ValueError('Expected both blocks at every integer exponent from 21 through 26')
    ...
    (HERE/'bch_k_counting_model_verification_v2.json').write_text(json.dumps(result, indent=2)+'\n')
