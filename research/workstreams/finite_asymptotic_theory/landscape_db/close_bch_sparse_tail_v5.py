...
def main():
    ...
    models = [fast.CompositionBoxes(counts, block, bands,
    ...
    directory = HERE/f'bch_sparse_tail_v5_b{block}_t{t}_s{s}_e{exponent}_q{args.minimum}_{maximum}'
    directory.mkdir(exist_ok=True)
    for path in (Path(__file__), Path(transfer.__file__), Path(positive.__file__), Path(lazy.__file__), Path(fast.__file__)):
    ...
    for q in range(args.minimum, maximum+1):
        saved = directory/f'occupation_{q}.json'
        if saved.exists():
            cached = json.loads(saved.read_text())
            if cached['input_fingerprint'] != fingerprint or cached['row']['occupation'] != q:
                raise ValueError('stale occupation checkpoint')
        ...
        row = dict(occupation=q, log_upper=value, adaptive_log_upper=float(best[q-1]),
        ...
        temporary.write_text(json.dumps(dict(input_fingerprint=fingerprint, row=row))+'\n')
