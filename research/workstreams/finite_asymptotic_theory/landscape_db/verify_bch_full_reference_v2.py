...
def main():
    ...
    dense_path = HERE/f'bch_dense_v5_b{block}_t{t}_s{s}_e{exponent}_q{args.dense_minimum}/cover.json'
    if not dense_path.exists():
        dense_path = HERE/f'bch_dense_v4_b{block}_t{t}_s{s}_e{exponent}_q{args.dense_minimum}/cover.json'
    if not dense_path.exists():
        dense_path = HERE/f'bch_dense_v3_b{block}_t{t}_s{s}_e{exponent}_q{args.dense_minimum}/cover.json'
    ...
    result = dict(status='VERIFIED_BINARY64_FULL_REFERENCE', block_bits=block, step_bits=t, state_bits=s,
                  ...
                  source_sha256={str(p): study.sha(p) for p in [Path(__file__), Path(lazy.__file__), checkpoint, *paths]},
