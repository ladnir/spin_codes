"""Outward pair-kernel tiles for actual overlaps161..200."""
import argparse
import math
...
def run(tile, verify=False):
    assert 0 <= tile < 3
    output = base.HERE/'generated'/f'mid_overlap_tile{tile:02d}.json'
    ...
    i, j = [(0, 0), (0, 1), (1, 1)][tile]
    xs, ys = ranges[i], ranges[j]
    ks = list(range(161, 201))
    if old:
        ps = list(map(base.decode, old['tilt_probabilities']))
    else:
        cx, cy, ck = 780+80*i, 780+80*j, 180
    ...
    for start in range(0, total, 2048):
        ...
        if (start+len(indices)) % (total//4) == 0:
            print('Mid-overlap tile', tile, 'grid', (start+len(indices))*100//total, 'percent', flush=True)
    ...
    if old:
        print('512-bit mid-overlap tile replay passed:', tile, flush=True)
        return
    ...
    base.write_new(output, dict(status='OUTWARD_ACTUAL_PAIR_MID_OVERLAP_TILE', configuration=tight.NAME,
    ...
    print('Mid-overlap tile', tile, 'intercept <=', float(intercept), 'common envelope passed:', intercept<=0,
