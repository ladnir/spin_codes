...
def run():
    output = base.HERE/'generated/adaptive_201_overlap_screen.json'
    assert not output.exists()
    source = base.HERE/'generated/global_overlap_optimized_screen.json'
    coverage_path = base.HERE/'generated/overlap_envelope_201_coverage.json'
    coverage = base.read(coverage_path)
    assert coverage['actual_overlaps'] == list(range(201))
    actual = np.array([r['log_ratio_upper_screen'] for r in base.read(source)['rows']])
    for k in range(201):
        actual[k] = float(base.decode(coverage['intercept_by_overlap'][k]))+.00012*max((k-740**2/8192)**2, (k-900**2/8192)**2)
