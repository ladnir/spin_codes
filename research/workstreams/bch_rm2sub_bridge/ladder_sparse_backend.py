"""Frozen sparse recurrence with selected-map instance identities through m24."""
import math
...
def sources():
    ...
    for path in (Path(__file__),Path(core.__file__),Path(refresh.__file__),Path(identity.__file__)):
...
def compute(task,precision=256,saved=None):
    spec=task['instance']
    core.require(spec==identity.instance(spec['message_exponent']),'Instance fingerprint mismatch')
