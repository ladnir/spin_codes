...
Roots are enclosed by Arb. A uniform absolute error 2^-40 covers each
...
def discrete_upper(p,powers,counts,grid):
    ...
    for start in range(0,M,2048):
        ...
        value=evaluate(values,powers,counts)
        ...
        scaled=up(up(magnitude*2**30)+2**-10)
...
def run(tag,verify=False):
    ...
    local=[Path(__file__),screen_path,base.HERE/'screen_pair_type_bound.py',base.HERE/'generated/t128_s15_pair_spectrum.json',
    ...
    base.write_new(path,dict(status='OUTWARD_ONE_SHARED_REGION_PAIR_TYPE',configuration=tight.NAME,
        region_type=m,grid=grid,tilt_probabilities=[base.encode(v) for v in ps],evaluation_absolute_error=base.encode(F(1,1<<40)),
