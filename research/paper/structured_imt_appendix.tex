...
Our verification code checks these properties and both spectra by exact enumeration.
...
Rows of a transfer matrix index the entering coordinate, so weighted
measures propagate as row vectors. Write $e_Z$ for the zero-state unit row
vector and $\1$ for the all-ones column vector.
...
The proof of Theorem~\ref{thm:structured-spin-scalable} in
Section~\ref{sec:structured-distance} combines these regimes and accounts
for ordinary and transposed work.
...
The \artifactlink{https://github.com/ladnir/permute_conv/blob/main/workstreams/inner_design/imt_asymptotic/README.md}{author-side IMT verification guide}
