...
a spectrum-controlled outer constituent. Table~\ref{tab:transposed-comparison}
includes both, with the separately measured original-BAA reference marked.
...
Except for the separate original-BAA reference, we rebuild all rows with
one compiler and measure them on the same Ryzen
...
Ryzen host; their raw logs are not retained in our comparison archive.
...
Table~\ref{tab:transposed-comparison}. The external rows other than the
separate original-BAA reference retain that campaign. The SPIN row instead uses the
