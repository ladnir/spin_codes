...
In the coding sections, all vector spaces are over $\F_2$.
The applications explicitly introduce an extension field.
Unless stated otherwise, we write vectors as rows. For
...
$H_2(q):=-q\log_2q-(1-q)\log_2(1-q)$, where $0\log_20:=0$. For integers $a,b$, we set $\binom{a}{b}:=0$ unless
...
$E:\F_2^K\longrightarrow\F_2^N$. Under the row-vector convention, there is a unique generator matrix
$G_E\in\F_2^{K\times N}$ satisfying $E(x)=xG_E$. The associated linear code is
the image $\mathcal C(E):=E(\F_2^K)\subseteq\F_2^N$.
