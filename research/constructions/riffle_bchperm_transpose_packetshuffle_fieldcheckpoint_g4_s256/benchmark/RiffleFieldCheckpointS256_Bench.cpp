...
namespace
{
	...
	constexpr u64 State256Blocks = 256;
	...
	using Field256Vector = std::array<__m256i, 4>;

	OC_FORCEINLINE __m256i packedLow64(__m256i value) noexcept
	{
		return _mm256_and_si256(
			value, _mm256_set_epi64x(0, -1, 0, -1));
	}

	OC_FORCEINLINE Field256Vector carrylessMultiply128(
		__m256i leftLow,
		__m256i leftHigh,
		__m256i rightLow,
		__m256i rightHigh) noexcept
	{
		const auto low = _mm256_clmulepi64_epi128(leftLow, rightLow, 0x00);
		const auto high = _mm256_clmulepi64_epi128(leftHigh, rightHigh, 0x00);
		const auto middle = _mm256_xor_si256(
			_mm256_clmulepi64_epi128(
				_mm256_xor_si256(leftLow, leftHigh),
				_mm256_xor_si256(rightLow, rightHigh), 0x00),
			_mm256_xor_si256(low, high));
		return {
			packedLow64(low),
			_mm256_xor_si256(_mm256_srli_si256(low, 8), packedLow64(middle)),
			_mm256_xor_si256(_mm256_srli_si256(middle, 8), packedLow64(high)),
			_mm256_srli_si256(high, 8)
		};
	}

	OC_FORCEINLINE Field256Vector fieldMultiply256Vector(
		const Field256Vector& value,
		const Field256Vector& coefficient) noexcept
	{
		const auto low = carrylessMultiply128(
			value[0], value[1], coefficient[0], coefficient[1]);
		const auto high = carrylessMultiply128(
			value[2], value[3], coefficient[2], coefficient[3]);
		const auto mixed = carrylessMultiply128(
			_mm256_xor_si256(value[0], value[2]),
			_mm256_xor_si256(value[1], value[3]),
			_mm256_xor_si256(coefficient[0], coefficient[2]),
			_mm256_xor_si256(coefficient[1], coefficient[3]));

		std::array<__m256i, 8> product = {
			low[0],
			low[1],
			_mm256_xor_si256(low[2],
				_mm256_xor_si256(mixed[0],
					_mm256_xor_si256(low[0], high[0]))),
			_mm256_xor_si256(low[3],
				_mm256_xor_si256(mixed[1],
					_mm256_xor_si256(low[1], high[1]))),
			_mm256_xor_si256(high[0],
				_mm256_xor_si256(mixed[2],
					_mm256_xor_si256(low[2], high[2]))),
			_mm256_xor_si256(high[1],
				_mm256_xor_si256(mixed[3],
					_mm256_xor_si256(low[3], high[3]))),
			high[2],
			high[3]
		};

		// Reduce modulo x^256 + x^10 + x^5 + x^2 + 1.  Descending
		// folds are required because reducing limb seven can add bits to
		// limb four.  Each vector contains two independent field elements.
		for (u64 index = 8; index-- > 4;)
		{
			const auto valueHigh = product[index];
			const auto foldLow = _mm256_xor_si256(
				_mm256_xor_si256(valueHigh, _mm256_slli_epi64(valueHigh, 2)),
				_mm256_xor_si256(
					_mm256_slli_epi64(valueHigh, 5),
					_mm256_slli_epi64(valueHigh, 10)));
			const auto foldHigh = _mm256_xor_si256(
				_mm256_srli_epi64(valueHigh, 62),
				_mm256_xor_si256(
					_mm256_srli_epi64(valueHigh, 59),
					_mm256_srli_epi64(valueHigh, 54)));
			product[index - 4] = _mm256_xor_si256(product[index - 4], foldLow);
			product[index - 3] = _mm256_xor_si256(product[index - 3], foldHigh);
		}
		return { product[0], product[1], product[2], product[3] };
	}

	Field256 fieldMultiply256Scalar(Field256 value, const Field256& coefficient) noexcept
	{
		Field256 result{};
		for (u64 bit = 0; bit < 256; ++bit)
		{
			if ((coefficient.limb[bit >> 6] >> (bit & 63)) & 1)
				for (u64 limb = 0; limb < 4; ++limb)
					result.limb[limb] ^= value.limb[limb];
			const u64 carry = value.limb[3] >> 63;
			for (u64 limb = 4; limb-- > 1;)
				value.limb[limb] =
					(value.limb[limb] << 1) | (value.limb[limb - 1] >> 63);
			value.limb[0] <<= 1;
			value.limb[0] ^= 0x425 & (0 - carry);
		}
		return result;
	}
	...
	template<void (*Transpose)(u64*)>
	void transformField256(block* state, const Field256& coefficient) noexcept
	{
		alignas(64) std::array<std::array<u64, 128>, 4> lanes;
		for (u64 fieldLimb = 0; fieldLimb < 4; ++fieldLimb)
		{
			for (u64 row = 0; row < 64; ++row)
			{
				const block value = state[fieldLimb * 64 + row];
				lanes[fieldLimb][row] =
					static_cast<u64>(_mm_cvtsi128_si64(value.mData));
				lanes[fieldLimb][64 + row] =
					static_cast<u64>(_mm_extract_epi64(value.mData, 1));
			}
			Transpose(lanes[fieldLimb].data());
			Transpose(lanes[fieldLimb].data() + 64);
		}

		if constexpr (Transpose == transpose64Avx2)
		{
			Field256Vector packedCoefficient;
			for (u64 fieldLimb = 0; fieldLimb < 4; ++fieldLimb)
				packedCoefficient[fieldLimb] = _mm256_set_epi64x(
					0, static_cast<long long>(coefficient.limb[fieldLimb]),
					0, static_cast<long long>(coefficient.limb[fieldLimb]));

			for (u64 bit = 0; bit < 64; ++bit)
			{
				Field256Vector packedValue;
				for (u64 fieldLimb = 0; fieldLimb < 4; ++fieldLimb)
					packedValue[fieldLimb] = _mm256_set_epi64x(
						0, static_cast<long long>(lanes[fieldLimb][64 + bit]),
						0, static_cast<long long>(lanes[fieldLimb][bit]));
				const auto product =
					fieldMultiply256Vector(packedValue, packedCoefficient);
				for (u64 fieldLimb = 0; fieldLimb < 4; ++fieldLimb)
				{
					lanes[fieldLimb][bit] = static_cast<u64>(
						_mm_cvtsi128_si64(_mm256_castsi256_si128(product[fieldLimb])));
					lanes[fieldLimb][64 + bit] = static_cast<u64>(
						_mm_cvtsi128_si64(
							_mm256_extracti128_si256(product[fieldLimb], 1)));
				}
			}
		}
		else
		{
			for (u64 lane = 0; lane < 128; ++lane)
			{
				Field256 value;
				for (u64 fieldLimb = 0; fieldLimb < 4; ++fieldLimb)
					value.limb[fieldLimb] = lanes[fieldLimb][lane];
				const auto product = fieldMultiply256Scalar(value, coefficient);
				for (u64 fieldLimb = 0; fieldLimb < 4; ++fieldLimb)
					lanes[fieldLimb][lane] = product.limb[fieldLimb];
			}
		}
		...
		for (u64 fieldLimb = 0; fieldLimb < 4; ++fieldLimb)
		{
			...
			Transpose(lanes[fieldLimb].data() + 64);
			for (u64 row = 0; row < 64; ++row)
				state[fieldLimb * 64 + row].mData = _mm_set_epi64x(
					static_cast<long long>(lanes[fieldLimb][64 + row]),
					static_cast<long long>(lanes[fieldLimb][row]));
		}
	}

	template<void (*Transform)(block*, const Field256&)>
	void checkpointTranspose256(
		block* word,
		u64 wordBlocks,
		const Field256* coefficients) noexcept
	{
		std::array<block, State256Blocks> state;
		state.fill(block(0, 0));
		const u64 epochs = wordBlocks / EpochBlocks;
		for (u64 epoch = epochs; epoch-- > 0;)
		{
			...
			for (u64 offset = EpochBlocks; offset-- > 0;)
			{
				...
				position[offset] = state[offset];
			}
		}
	}

	void checkpoint256Optimized(
		block* word,
		u64 wordBlocks,
		const Field256* coefficients) noexcept
	{
		checkpointTranspose256<transformField256<transpose64Avx2>>(
			word, wordBlocks, coefficients);
	}

	void checkpoint256Reference(
		block* word,
		u64 wordBlocks,
		const Field256* coefficients) noexcept
	{
		checkpointTranspose256<transformField256<transpose64Scalar>>(
			word, wordBlocks, coefficients);
	}
	...
	public:
		...
		void dualEncodeS256Unchecked(
			const block* __restrict input,
			block* __restrict output,
			const Field256* __restrict coefficients,
			Workspace& workspace) const noexcept
		{
			std::array<block, State256Blocks> state;
			state.fill(block(0, 0));
			...
			{
				if (epoch + 1 < Epochs)
					transformField256<transpose64Avx2>(
						state.data(), coefficients[epoch]);
				const u64 epochBase = epoch * EpochBlocks;
				for (u64 offset = EpochBlocks; offset != 0; offset -= PacketWidth)
				{
					const u64 innerBase = epochBase + offset - PacketWidth;
					...
					for (u64 lane = PacketWidth; lane-- > 0;)
					{
						const u64 stateLane = offset - PacketWidth + lane;
						state[stateLane] ^= input[innerBase + lane];
						destination[lane] = state[stateLane];
					}
				}
			...
			for (u64 bucket = 0; bucket < Buckets; ++bucket)
			{
				...
				for (u64 packet = 0; packet < TilePackets; ++packet)
				{
					...
					const u64 route = mSlotRoute[slotPacket];
					const u64 localGroup = route & (TileOuterGroups - 1);
					const block* __restrict source =
						values + slotPacket * PacketWidth;
					for (u64 lane = 0; lane < PacketWidth; ++lane)
					{
						const u64 coordinate = (route >> (9 + 8 * lane)) & 255;
						tile[(localGroup * PacketWidth + lane) * OuterLength + coordinate] =
	...
	void selfTest256(const std::vector<Field256>& coefficients)
	{
		u64 randomState = 0x733235362d746573ULL;
		std::array<block, State256Blocks> optimizedState;
		for (auto& value : optimizedState)
			value = randomBlock(randomState);
		auto referenceState = optimizedState;
		transformField256<transpose64Avx2>(optimizedState.data(), coefficients[0]);
		transformField256<transpose64Scalar>(referenceState.data(), coefficients[0]);
		if (!equalBlocks(
			optimizedState.data(), referenceState.data(), State256Blocks))
			throw std::runtime_error(
				"optimized GF(2^256) transform disagrees with scalar reference");

		constexpr u64 testBlocks = 3 * EpochBlocks;
		std::array<block, testBlocks> optimizedWord;
		for (auto& value : optimizedWord)
			value = randomBlock(randomState);
		auto referenceWord = optimizedWord;
		checkpoint256Optimized(
			optimizedWord.data(), testBlocks, coefficients.data());
		checkpoint256Reference(
			referenceWord.data(), testBlocks, coefficients.data());
		if (!equalBlocks(optimizedWord.data(), referenceWord.data(), testBlocks))
			throw std::runtime_error(
				"optimized s=256 checkpoint disagrees with scalar reference");
	}
...
int main(int argc, char** argv)
{
	try
	{
		...
		const bool packet4S256Only =
			argc > 2 && std::string(argv[2]) == "packet4-s256";
		const bool packet4S256CounterOnly =
			argc > 2 && std::string(argv[2]) == "packet4-s256-counter";
#if defined(LIBOTE_RIFFLE_BCH_CIRCUIT_PAIR_BENCH)
		...
		std::vector<Field256> coefficients256(Epochs - 1);
		for (auto& coefficient : coefficients256)
		{
			u64 nonzero = 0;
			for (auto& limb : coefficient.limb)
			{
				limb = splitmix64(randomState);
				nonzero |= limb;
			}
			coefficient.limb[0] |= static_cast<u64>(nonzero == 0);
		}
		if (packet4S256Only || packet4S256CounterOnly)
			selfTest256(coefficients256);
		...
		if (packet4S256Only || packet4S256CounterOnly)
		{
			...
			checkpoint256Optimized(
				working.data(), N, coefficients256.data());
			packet.gather(working.data(), outerWord.data());
			outerTranspose2(outerWord.data(), message.data());
			const auto packetReference = message;
			packet.dualEncodeS256Unchecked(
				source.data(), message.data(), coefficients256.data(), packetWorkspace);
			if (!equalBlocks(packetReference.data(), message.data(), K))
				throw std::runtime_error(
					"s=256 packet evaluator disagrees with its flat oracle");
			...
			const auto packetFull = benchmark(
				...
				[&] {
					packet.dualEncodeS256Unchecked(
						working.data(), message.data(),
						coefficients256.data(), packetWorkspace);
				});
			if (!equalBlocks(packetReference.data(), message.data(), K))
				throw std::runtime_error(
					"timed s=256 packet evaluator disagrees with its oracle");
			benchmarkSink ^= checksum(message.data(), message.size());
			std::cout << std::fixed << std::setprecision(6)
				<< "construction=Riffle-BCHPerm-TransposePacketShuffle-FieldCheckpoint-g4-s256"
				<< " mode=" << (packet4S256CounterOnly
					? "packet4-s256-counter" : "packet4-s256")
				<< " benchmark_cpu=" << benchmarkCpu
				<< " K=" << K << " N=" << N
				<< " packet_width=4 packets_per_region=2048"
				<< " state_bits=256 epoch_bits=256"
				<< " field_modulus=x^256+x^10+x^5+x^2+1"
				<< " field_kernel=vpclmul-karatsuba9"
				<< " setup_ms=" << packetSetupMs
				<< " hot_schedule_mib=5.500000"
				<< " workspace_mib=40.000000"
				<< " trials=" << trials << " correctness=PASS\n";
			printSamples("packet4_s256_full", packetFull);
