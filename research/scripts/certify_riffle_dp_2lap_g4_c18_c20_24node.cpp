...
namespace
{
    ...
    std::uint64_t generatorInverse()
    {
        ...
        if (carrylessMultiplyLow(Generator, inverse) != 1)
            throw std::runtime_error("c18+c20 24-node certificate: generator inverse failed");
        return inverse;
    }
    ...
    std::array<std::uint64_t, 64> systematicRightColumns()
    {
        ...
        for (unsigned column = 0; column < 64; ++column)
        {
            ...
            if (word.low != (std::uint64_t{1} << column))
                throw std::runtime_error("c18+c20 24-node certificate: systematic BCH failed");
        ...
        for (unsigned column = 0; column < 64; ++column)
        {
            ...
            if (value == 0)
            {
                if (basisSize >= Dimension)
                    throw std::runtime_error("c18+c20 24-node certificate: oversized kernel");
                basis[basisSize++] = representation;
            }
        }
        if (basisSize != Dimension)
            throw std::runtime_error("c18+c20 24-node certificate: kernel dimension changed");
        return basis;
    }
        ...
        for (unsigned attempt = 0; attempt < 10'000; ++attempt)
        {
            ...
            if (!failed)
            {
                if (availableSize != result.unused.size())
                    throw std::runtime_error("c18+c20 24-node certificate: unused coordinate count");
                std::copy_n(available.begin(), availableSize, result.unused.begin());
                result.restarts = attempt;
                return result;
            }
        }
        throw std::runtime_error("c18+c20 24-node certificate: ten information sets not found");
    }
        ...
        const InformationSet& informationSet)
    {
        ...
        for (unsigned column = 0; column < Dimension; ++column)
        {
            ...
            while (pivot < Dimension && !coordinate(systematic[pivot].word, informationSet[column]))
                ++pivot;
            if (pivot == Dimension)
                throw std::runtime_error("c18+c20 24-node certificate: singular information set");
        ...
        for (unsigned row = 0; row < Dimension; ++row)
        {
            for (unsigned column = 0; column < Dimension; ++column)
            {
                if (coordinate(systematic[row].word, informationSet[column]) != (row == column))
                    throw std::runtime_error("c18+c20 24-node certificate: systematic verification failed");
            }
        }
        return systematic;
    }
        ...
        const std::array<std::vector<std::uint32_t>, HalfDimension + 1>& masks)
    {
        ...
        if (result.candidates != ballSize(radius))
            throw std::runtime_error("c18+c20 24-node certificate: Hamming ball count mismatch");
        return result;
    }
        ...
        double elapsedSeconds)
    {
        ...
        if (!output)
            throw std::runtime_error("c18+c20 24-node certificate: cannot open receipt");
        output << "{\n";
        output << "  \"schema\": \"riffle-dp-2lap-g4-c18-c20-24node-certificate-v1\",\n";
        ...
        output << "  \"proof_rule\": \"Ten disjoint information sets cover 380 coordinates. A word or complement of weight below the required distance restricts to Hamming radius floor((d-1)/10) in at least one set. Each restriction determines one codeword. The search exhausts both balls around zero and one.\",\n";
...
int main(int argc, char** argv)
{
    try
    {
        if (argc != 3)
        {
            std::cerr << "usage: certify_riffle_dp_2lap_g4_c18_c20_24node <packet-value> <output.json>\n";
            return 2;
        }
        const unsigned packetValue = std::stoul(argv[1]);
        if (packetValue < 1 || packetValue > 15)
            throw std::runtime_error("c18+c20 24-node certificate: packet value outside 1..15");
