...
namespace
{
    ...
    constexpr unsigned WindowNodes = 24;
    constexpr unsigned Slots = 16;
    constexpr unsigned Length = WindowNodes * Slots;
    constexpr unsigned InformationSetCount = 10;
    ...
    std::uint64_t generatorInverse()
    {
        ...
        if (carrylessMultiplyLow(Generator, inverse) != 1)
            throw std::runtime_error("c18+c20 24-node verifier: generator inverse failed");
        return inverse;
    }
    ...
    std::array<std::uint64_t, 64> systematicRightColumns()
    {
        ...
        for (unsigned column = 0; column < 64; ++column)
        {
            ...
            if (word.low != (std::uint64_t{1} << column))
                throw std::runtime_error("c18+c20 24-node verifier: systematic BCH failed");
        ...
        for (unsigned column = 0; column < 64; ++column)
        {
            ...
            if (value == 0)
            {
                if (basisSize >= Dimension)
                    throw std::runtime_error("c18+c20 24-node verifier: oversized kernel");
                basis[basisSize++] = representation;
            }
        }
        if (basisSize != Dimension)
            throw std::runtime_error("c18+c20 24-node verifier: kernel dimension changed");
        return basis;
    }
        ...
        const std::array<std::uint64_t, Length>& columns)
    {
        ...
        std::mt19937_64 random(0x24C1820BEEFULL + packetValue);
        for (unsigned attempt = 0; attempt < 10'000; ++attempt)
        {
            ...
            if (!failed)
            {
                if (availableSize != result.unused.size())
                    throw std::runtime_error("c18+c20 24-node verifier: unused coordinate count");
                std::copy_n(available.begin(), availableSize, result.unused.begin());
                result.restarts = attempt;
                return result;
            }
        }
        throw std::runtime_error("c18+c20 24-node verifier: ten information sets not found");
    }
        ...
        const InformationSet& informationSet)
    {
        ...
        for (unsigned column = 0; column < Dimension; ++column)
        {
            ...
            while (pivot < Dimension && !coordinate(systematic[pivot].word, informationSet[column]))
                ++pivot;
            if (pivot == Dimension)
                throw std::runtime_error("c18+c20 24-node verifier: singular information set");
        ...
        for (unsigned row = 0; row < Dimension; ++row)
        {
            for (unsigned column = 0; column < Dimension; ++column)
            {
                if (coordinate(systematic[row].word, informationSet[column]) != (row == column))
                    throw std::runtime_error("c18+c20 24-node verifier: systematic verification failed");
            }
        }
        return systematic;
    }
        ...
        SearchResult& result)
    {
        ...
        const Word384 highWord = allOnesWord ^ word;
        ...
        unsigned requiredDistance)
    {
        ...
        if (result.candidates != ballSize(radius))
            throw std::runtime_error("c18+c20 24-node verifier: Hamming ball count mismatch");
        return result;
    }
        ...
        double elapsedSeconds)
    {
        ...
        if (!output)
            throw std::runtime_error("c18+c20 24-node verifier: cannot open receipt");
        output << "{\n";
        output << "  \"schema\": \"riffle-dp-2lap-g4-c18-c20-24node-independent-verification-v1\",\n";
        output << "  \"candidate\": \"Riffle DP-2Lap g=4\",\n";
        output << "  \"evidence_label\": \""
               << (passed ? "EXACT_INDEPENDENT_EXHAUSTIVE_VERIFICATION" : "EXACT_INDEPENDENT_COUNTEREXAMPLE")
               << "\",\n";
        ...
        output << "  \"proof_rule\": \"Ten disjoint information sets cover 380 coordinates. A violating word or complement restricts to radius floor((d-1)/10) in one set. This verifier uses direct recursive combination enumeration over both balls and a partition seed distinct from the primary certificate.\",\n";
...
int main(int argc, char** argv)
{
    try
    {
        if (argc != 3)
        {
            std::cerr << "usage: verify_riffle_dp_2lap_g4_c18_c20_24node <packet-value> <output.json>\n";
            return 2;
        }
        const unsigned packetValue = std::stoul(argv[1]);
        if (packetValue < 1 || packetValue > 15)
            throw std::runtime_error("c18+c20 24-node verifier: packet value outside 1..15");
        const unsigned requiredDistance = packetValue == 15 ? 73 : 97;
